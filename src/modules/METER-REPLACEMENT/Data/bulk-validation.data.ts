...
export const bulkValidateMeterReplacementMaxResponseTimeMs = METER_REPLACEMENT_MAX_RESPONSE_TIME_MS;
...
export function buildValidMeterReplacementBulkRow(
  ...
  return {
    "Sl No": 1,
    "Old Meter Serial": resolveMeterReplacementEnv("METER_REPLACEMENT_OLD_METER_SERIAL"),
    "Old Meter Reading": 1000,
    "New Meter Serial": resolveMeterReplacementEnv("METER_REPLACEMENT_NEW_METER_SERIAL"),
    "New Meter Reading": 0,
...
export interface BulkValidateMeterReplacementTestCase {
  ...
  buildUpload: (authenticatedApi: APIRequestContext) => Promise<BulkUploadFileInput>;
...
export const bulkValidateMeterReplacementTestCases: BulkValidateMeterReplacementTestCase[] = [
  // ─── File validation ──────────────────────────────────────────────────────
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — only .xlsx allowed",
  ...
  },
  {
    ...
    expectedStatus: 400,
    buildUpload: async () => {
      const columns = METER_REPLACEMENT_BULK_COLUMNS.filter((c) => c !== "Old Meter Serial");
      const buffer = await buildMeterReplacementBulkXlsx([buildValidMeterReplacementBulkRow()], {
        columns: [...columns],
      });
  ...
  },
  {
    ...
    buildUpload: async () => {
      ...
      return xlsxUpload(buffer, "meter-replacement-bulk-duplicate-columns.xlsx");
    },
    tags: ["@meter-replacement", "@bulk-validate", "@negative"],
    nonEmptyExpected: false,
  },
  ...
  },
  {
    ...
    buildUpload: async (authenticatedApi) => {
      ...
      return xlsxUpload(await buildMeterReplacementBulkXlsx([valid, invalid]));
    },
  ...
  // ─── Row-level: old meter ─────────────────────────────────────────────────
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — old meter serial required",
  ...
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — inactive old meter rejected",
    scenario: "row_old_meter_inactive",
    expectedStatus: 200,
    envKeys: ["METER_REPLACEMENT_INACTIVE_METER_SERIAL"],
    buildUpload: async (authenticatedApi) => {
      const row = await buildRuntimeRow(authenticatedApi, {
        "Old Meter Serial": resolveMeterReplacementEnv("METER_REPLACEMENT_INACTIVE_METER_SERIAL"),
      });
      return xlsxUpload(await buildMeterReplacementBulkXlsx([row]));
    },
    tags: ["@meter-replacement", "@bulk-validate", "@negative"],
  ...
  // ─── Row-level: new meter ─────────────────────────────────────────────────
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — new meter serial required",
  ...
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — new meter serial must exist",
  ...
  },
  {
    ...
    buildUpload: async (authenticatedApi) => {
      const runtime = await ensureMeterReplacementBulkRuntimeContext(authenticatedApi);
      const assigned =
        (await resolveAssignedNewMeterSerial(authenticatedApi, runtime.oldMeterSerial)) ||
  ...
  // ─── Row-level: business rules ────────────────────────────────────────────
  {
    ...
    expectedStatus: 200,
    buildUpload: async (authenticatedApi) => {
      const pendingOld = await resolvePendingConsumerOldMeterSerial(authenticatedApi);
      if (!pendingOld) {
        test.skip(
          true,
          "No PENDING consumer old-meter serial available. Set METER_REPLACEMENT_PENDING_CONSUMER_OLD_SERIAL or seed a PENDING replacement.",
        );
      }
      const runtime = await ensureMeterReplacementBulkRuntimeContext(authenticatedApi);
      const row = buildValidMeterReplacementBulkRow({
  ...
  {
    testName: "Validate POST /indore/meter-replacement/bulk/validate — replacement reason required",
