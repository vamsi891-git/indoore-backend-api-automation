...
test.describe("Meter Replacement Consumer Search API — Negative & Edge", () => {
  test(
    "Empty and whitespace search return validation error",
    {
    ...
    async ({ authenticatedApi }) => {
      ...
      for (const search of [consumerSearchData.emptySearch, consumerSearchData.whitespaceSearch]) {
  ...
  test(
    "Unknown search returns consumer not found",
    {
      tag: ["@meter-replacement", "@consumer-search", "@negative"],
    },
    async ({ authenticatedApi }) => {
  ...
  test(
    "Case and identity variants return matching consumers",
    {
