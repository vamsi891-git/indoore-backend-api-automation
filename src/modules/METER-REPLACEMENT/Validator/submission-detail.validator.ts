...
export class SubmissionDetailValidator {
  ...
  validateSubmissionStatus(data: SubmissionDetailData) {
    expect(ALLOWED_SUBMISSION_STATUS).toContain(
      data.status,
    );
  }
  ...
  validateCreatedDate(data: SubmissionDetailData) {
    ...
    expect(
      DATE_PATTERN.test(data.createdDate),
    ).toBeTruthy();
  }
  ...
  validateCompletedDate(data: SubmissionDetailData) {
    ...
    expect(
      DATE_PATTERN.test(data.completedDate),
    ).toBeTruthy();
  }

  validateConsumerObject(
    consumer: SubmissionConsumer,
  ) {
    expect(typeof consumer).toBe("object");
  }

  validateConsumerRequiredFields(
    consumer: SubmissionConsumer,
  ) {
  ...
  validateConsumerName(
    ...
    expect(
      consumer.consumerName.trim().length,
    ).toBeGreaterThan(0);
  }

  validateConsumerStatus(
    consumer: SubmissionConsumer,
  ) {
    expect(
      ALLOWED_CONSUMER_STATUS,
    ).toContain(
  ...
  validateConsumerIdentity(
    ...
    expect(
      consumer.consumerCid.length,
    ).toBeGreaterThan(0);

    expect(
      consumer.accountId.length,
    ).toBeGreaterThan(0);

    expect(
      consumer.servicePointId.length,
    ).toBeGreaterThan(0);

  }

  validateOldMeterObject(
    meter: SubmissionMeter,
  ) {
    expect(typeof meter).toBe("object");
  }

  validateOldMeterRequiredFields(
    meter: SubmissionMeter,
  ) {
  ...
  validateOldMeterLookupId(
    meter: SubmissionMeter,
  ) {

    expect(
      typeof meter.meterLookupId,
    ).toBe("number");
  ...
  validateOldMeterSerial(
    meter: SubmissionMeter,
  ) {

    expect(
      meter.meterSerial.trim().length,
    ).toBeGreaterThan(0);

  }

  validateOldMeterStatus(
    meter: SubmissionMeter,
  ) {

    expect(
      ALLOWED_METER_STATUS,
    ).toContain(
      meter.meterStatus,
    );
  ...
  validateNewMeterObject(
    meter: SubmissionMeter,
  ) {
    expect(typeof meter).toBe("object");
  }

  validateNewMeterRequiredFields(
    meter: SubmissionMeter,
  ) {
  ...
  validateConsumerTrim(
    consumer: SubmissionConsumer,
  ) {
    expect(consumer.consumerCid).toBe(
      consumer.consumerCid.trim(),
    );
    expect(consumer.consumerName).toBe(
      consumer.consumerName.trim(),
    );
    expect(consumer.ivrs).toBe(
      consumer.ivrs.trim(),
    );
    expect(consumer.rrNumber).toBe(
      consumer.rrNumber.trim(),
    );
    expect(consumer.accountId).toBe(
      consumer.accountId.trim(),
    );
    expect(consumer.servicePointId).toBe(
      consumer.servicePointId.trim(),
    );
    expect(consumer.address).toBe(
      consumer.address.trim(),
    );
    expect(consumer.zone).toBe(
      consumer.zone.trim(),
    );
    expect(consumer.office).toBe(
      consumer.office.trim(),
    );
  }

  validateOldMeterTrim(
    meter: SubmissionMeter,
  ) {
    expect(meter.meterSerial).toBe(
      meter.meterSerial.trim(),
    );

    if (meter.meterReading != null) {
      expect(meter.meterReading).toBe(
        meter.meterReading.trim(),
      );
    }
  }

  validateNewMeterTrim(
    meter: SubmissionMeter,
  ) {
    expect(meter.meterSerial).toBe(
      meter.meterSerial.trim(),
  ...
  validateReplacementReason(
    ...
    expect(
      data.replacementReason.trim().length,
    ).toBeGreaterThan(0);
  ...
  validateSubmittedByTrim(
    data: SubmissionDetailData,
  ) {
    expect(data.submittedBy).toBe(
      data.submittedBy.trim(),
    );
  }
