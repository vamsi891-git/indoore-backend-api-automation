...
import { ProgressChart, ProgressData,} from "../Mapper/progress.mapper";
...
export class ProgressValidator {
  validateSuccess(success: boolean) {expect(success).toBeTruthy();}
  validateRootStructure(data: ProgressData & { success?: boolean },) {
    expect(typeof data).toBe("object");
  }
  validateRequiredRootFields(data: ProgressData,) {
  ...
  validateWeeklyBucketCount(weekly: ProgressChart) {
    expect(weekly.labels.length).toBe(progressData.weeklyBucketCount,);
    expect(weekly.values.length).toBe(progressData.weeklyBucketCount,);
  }
  validateMonthlyBucketCount(monthly: ProgressChart) {
    expect(monthly.labels.length).toBe(progressData.monthlyBucketCount,);
    expect(monthly.values.length).toBe(progressData.monthlyBucketCount,);
  }
  validateWeeklyLabels(weekly: ProgressChart) {
    expect(weekly.labels).toEqual([...progressData.expectedWeeklyLabels,]);
  }
  validateMonthlyLabels(monthly: ProgressChart) {
    expect(monthly.labels).toEqual([...progressData.expectedMonthlyLabels,]);
  }
  ...
  validateValuesNonNegative(chart: ProgressChart) {
    chart.values.forEach((value) => {
      expect(value).toBeGreaterThanOrEqual(progressData.minimumCount,);
    });
  }
  validateValuesSafeInteger(chart: ProgressChart) {
    chart.values.forEach((value) => {
      expect(value).toBeLessThanOrEqual(progressData.maximumSafeInteger,);
    });
  }
  ...
  validateChartNoExtraFields(chart: ProgressChart) {
    expect(Object.keys(chart).sort()).toEqual(["labels","values",]);
  }
  validateRootObjectSize(data: ProgressData & { success?: boolean },) {
    expect(Object.keys(data).length).toBe(3);
  }
  validateNoExtraRootFields(data: ProgressData & { success?: boolean },) {
    expect(Object.keys(data).sort()).toEqual(["monthly","success","weekly",]);
  }
  ...
  validateZeroProgressScenario(data: ProgressData) {
    ...
    if (weeklySum === 0) {
      data.weekly.values.forEach((value) => {expect(value).toBe(0);});
    }
    if (monthlySum === 0) {
  ...
  validateWeeklyLabelFormat(weekly: ProgressChart) {weekly.labels.forEach((label) => {
      expect(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)$/.test(label),).toBeTruthy();
    });
  }
  ...
  validateWeeklyValuesCountMatch(weekly: ProgressChart) {
    expect(weekly.values.length).toBe(weekly.labels.length,);
  }
  validateMonthlyValuesCountMatch(monthly: ProgressChart) {
    expect(monthly.values.length).toBe(monthly.labels.length,);
  }
  ...
  validateBusinessRules(data: ProgressData) {
    ...
    expect(data.weekly.labels.length).toBe(progressData.weeklyBucketCount,);
  ...
  validateWeeklyOrderedLabels(weekly: ProgressChart) {
    expect(weekly.labels.join(",")).toBe(progressData.expectedWeeklyLabels.join(","),);
  }
  validateMonthlyOrderedLabels(monthly: ProgressChart) {
    expect(monthly.labels.join(",")).toBe(progressData.expectedMonthlyLabels.join(","),);
  }
  validateNoExtraWeeklyLabel(weekly: ProgressChart,) {
    weekly.labels.forEach((label) => {
      expect(progressData.expectedWeeklyLabels,).toContain(label);
    });
  }
  validateNoExtraMonthlyLabel(monthly: ProgressChart,) {
    monthly.labels.forEach((label) => {
      expect(progressData.expectedMonthlyLabels,).toContain(label);
    });
  }
  ...
  validateUnauthorizedError(
    ...
    expect(status).toBe(progressData.expectedUnauthorizedStatus,);
    expect(body.success).toBeFalsy();
    expect([progressData.expectedUnauthorizedCode,progressData.expectedInvalidTokenCode,]).toContain(body.error?.code);
  ...
  validateSuccessEnvelopePreserved(body: ProgressResponseLike,) {
