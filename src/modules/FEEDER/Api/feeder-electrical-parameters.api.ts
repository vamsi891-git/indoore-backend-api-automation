...
export class FeederElectricalParametersApi {
    ...
    async getElectricalParameters(feederCode: string): Promise<FeederElectricalParametersApiResult> {
        const start = Date.now();
        const response =await getWithAutoRefresh(this.authenticatedApi,`/indore/feeder/${feederCode}/electrical-parameters`);
        return {
