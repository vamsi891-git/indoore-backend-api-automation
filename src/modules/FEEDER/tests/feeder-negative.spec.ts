  ...
  for (const testCase of feederNegativeCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.kind === "profile") {
        const result = await new FeederProfileApi(authenticatedApi).getFeederProfile(code);
        status = result.rawResponse.status();
        body = result.responseBody;
        skipIfFeederInternalError(status, body, `/indore/feeder/${code}/profile`);
        ...
        status = result.rawResponse.status();
        body = result.responseBody;
        skipIfFeederInternalError(status, body, `/indore/feeder/${code}/alerts`);
      } else {
        const result = await new FeederDailyConsumptionApi(authenticatedApi).getDailyConsumption(
          code,
          String(params.granularity) as "day",
        );
        status = result.rawResponse.status();
        body = result.responseBody;
        skipIfFeederInternalError(status, body, `/indore/feeder/${code}/daily-consumption`);
      }
