...
test.describe("Feeder daily energy", () => {
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { granularity, expectedUnit, maxResponseTime } =
        feederDailyConsumptionData;
      const feederCode = resolveFeederCode(
      ...
      if (rawResponse.status() === 200) {
        validation.execute("Zod Response Schema", () => {
          const result =
            FeederDailyConsumptionSuccessResponseSchema.safeParse(responseBody);
          expect(
            result.success,
            result.success
      ...
      validation.execute("Day Point Count", () =>
        validator.validateDayPointCount(points),
      );
