    ...
    async ({ authenticatedApi }) => {
      ...
      if (rawResponse.status() === 200) {
        validation.execute("Zod Response Schema", () => {
          const result = FeederAlertsSuccessResponseSchema.safeParse(responseBody);
          expect(
            result.success,
            result.success
      ...
      if (rows.length > 0) {
        ...
        validation.execute("Row Required Fields", () => validator.validateRowRequiredFields(rows));
        validation.execute("Row Structure", () => validator.validateRowStructure(rows));
        ...
        validation.execute("Status Rules", () => validator.validateStatusRules(rows));
        validation.execute("Status Distribution", () => validator.validateStatusDistribution(rows));
        validation.execute("Meter Number", () => validator.validateMeterNumber(rows));
        validation.execute("Event Type", () => validator.validateEventType(rows));
        validation.execute("Occurred On Format", () => validator.validateOccurredOnFormat(rows));
        validation.execute("Duration Format", () => validator.validateDurationFormat(rows));
        validation.execute("Chronological Order", () => validator.validateChronologicalOrder(rows));
      }
