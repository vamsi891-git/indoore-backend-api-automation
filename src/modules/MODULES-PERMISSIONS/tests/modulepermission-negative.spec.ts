...
test.describe("Module Permission — Negative", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await moduleApi.createModule(
        ModulePermissionData.invalidModuleKeyPayload,
      );
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const moduleApi = new ModulePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await moduleApi.createModule(
        ModulePermissionData.invalidModuleNamePayload,
      );
      skipIfRateLimited(rawResponse.status());
      validation.execute("Status (validation error)", () =>
        assert.validateStatusCode(rawResponse, 400, responseBody),
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await moduleApi.updateModule(
        moduleId,
        {},
      );
      skipIfRateLimited(rawResponse.status());
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await moduleApi.deleteModule(
        protectedModule!.id,
      );
      const status = rawResponse.status();
      skipIfRateLimited(status);
      if (
        BackendResponse.shouldSkipServerFailure(status, "Delete Protected Module", responseBody)
      ) {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      skipIfRateLimited(created.rawResponse.status());
      const moduleId = ModulePermissionMapper.mapModule(created.responseBody).id;
      const { rawResponse, responseBody, responseTime } = await moduleApi.createPermission(
        moduleId,
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const validator = new ModulePermissionValidator();
      const moduleApi = new ModulePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await moduleApi.updatePermission(
        ModulePermissionData.unknownResourceId,
        {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const moduleKey = ModulePermissionMapper.mapModule(createdModule.responseBody).key;
      const moduleId = ModulePermissionMapper.mapModule(createdModule.responseBody).id;
      const permissionPayload = ModulePermissionData.buildPermissionPayload(moduleKey, "view");
      ...
      const duplicate = await moduleApi.createPermission(moduleId, permissionPayload);
      skipIfRateLimited(duplicate.rawResponse.status());
      ...
      const permissionId = ModulePermissionMapper.mapPermission(first.responseBody).id;
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await moduleApi.deletePermission(
        ModulePermissionData.unknownResourceId,
      );
