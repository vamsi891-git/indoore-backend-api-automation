...
test.describe("Module Permission — Negative", () => {
  ...
  test("POST /permissions/modules — invalid module key returns 400",
  ...
  test("POST /permissions/modules — invalid module name returns 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
  ...
  test("POST /permissions/modules — duplicate module key returns 409 or 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
      ...
      validation.printSummary("Create Module — Duplicate Key",first.responseTime + duplicate.responseTime,);
    },
  );
  test("PATCH /permissions/modules/:id — empty body returns 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
  ...
  test("PATCH /permissions/modules/:id — unknown module returns 404",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
      ...
      const moduleApi = new ModulePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } =
        await moduleApi.updateModule(ModulePermissionData.unknownResourceId, { name: "Does Not Exist", });
      skipIfRateLimited(rawResponse.status());
  ...
  test("DELETE /permissions/modules/:id — protected module returns 403",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
      ...
      if (BackendResponse.shouldSkipServerFailure(status,"Delete Protected Module",responseBody,)
      ) {
      ...
      validation.execute("Error envelope", () =>
        validator.validateErrorResponse(status,responseBody,[403],"PROTECTED_RESOURCE",),
      );
      validation.printSummary("Delete Protected Module", responseTime);
    },
  );
  test("POST /permissions/modules/:id/permissions — invalid key (no dot) returns 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
      ...
      const moduleId = ModulePermissionMapper.mapModule(created.responseBody).id;
      const { rawResponse, responseBody, responseTime } =
        await moduleApi.createPermission(moduleId,ModulePermissionData.invalidPermissionKeyNoDot,);
      skipIfRateLimited(rawResponse.status());
  ...
  test("POST /permissions/modules/:id/permissions — key prefix mismatch returns 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
  ...
  test("POST /permissions/modules/:id/permissions — unknown module returns 404",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
  ...
  test("PATCH /permissions/permissions/:id — unknown permission returns 404",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
  ...
  test("POST /permissions/modules/:id/permissions — duplicate permission key returns 409 or 400",
    { tag: ["@negative", "@permissions", "@modules-permissions"] },
    async ({ authenticatedApi }) => {
      const assert = new AssertionEngine();
      const validation = new ValidationEngine();
