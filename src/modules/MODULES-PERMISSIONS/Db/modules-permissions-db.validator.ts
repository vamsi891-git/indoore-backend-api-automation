...
export function collectModulesPermissionsDataQualityFindings(
  kind: string,
...
export async function logModulesPermissionsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectModulesPermissionsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `MODULES-PERMISSIONS ${kind} DQ`);
  return report;
}
