...
export function collectAuthDataQualityFindings(
  kind: string,
...
export async function logAuthDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectAuthDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `AUTH ${kind} DQ`);
  return report;
}
