...
import { AuthErrorResponseSchema, InviteAcceptRequestSchema } from "../schemas/auth.schemas";
  ...
  test(
    ...
    async ({ unauthenticatedApi }) => {
      ...
      try {
        ...
        const accept = await publicApi.acceptInvitation(acceptPayload, csrfToken);
        ...
        validation.execute("Invalid Accept Zod", () => {
          ...
          expect(result.data?.error.code).toBe(InviteTestData.acceptErrorCodes.invalid);
        });
      } finally {
