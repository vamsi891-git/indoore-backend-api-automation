...
test.describe.skip("Auth Invite Preview API", () => {
  test("Validate invalid invitation preview", { tag: ["@smoke", "@auth", "@invite"] }, async () => {
    ...
    try {
      const preview = await api.previewInvitation(InviteTestData.invalidPreviewToken);
      ...
      validation.execute("Invalid Preview Zod", () => {
        ...
        expect(result.data?.error.code).toBe(InviteTestData.previewErrorCodes.invalid);
      });
    ...
    test(
      ...
      async () => {
        ...
        try {
          ...
          validation.finalize("Auth Invite Preview API (valid)", preview.responseTime);
        } finally {
          await publicCtx.dispose();
        }
      },
