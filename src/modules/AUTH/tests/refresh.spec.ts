...
test.describe("Auth Refresh API", () => {
  test("Validate token refresh", { tag: ["@smoke", "@auth"] }, async ({ unauthenticatedApi }) => {
    ...
    validation.execute("Refresh CSRF", () => validator.validateCsrfToken(csrfToken));
    ...
    try {
      validation.execute("Refresh Status", () =>
        assert.validateStatusCode(refresh.rawResponse, 200, refresh.responseBody),
      );
      ...
      validation.execute("Refresh Response Time", () =>
        assert.validateResponseTime(refresh.responseTime, AuthTestData.maxResponseTimeMs),
      );
      ...
      if (refresh.rawResponse.status() === 200) {
        validation.execute("Refresh Zod", () => {
          const result = AuthRefreshSuccessResponseSchema.safeParse(refresh.responseBody);
          expect(
            result.success,
            result.success
        ...
        const parsed = AuthRefreshSuccessResponseSchema.parse(refresh.responseBody);
        const afterSession = AuthMapper.mapLoginSession(parsed.data, csrfToken);
        ...
        TokenManager.seed(afterSession.accessToken, afterSession.expiresIn, afterSession.csrfToken);
      }
    } finally {
