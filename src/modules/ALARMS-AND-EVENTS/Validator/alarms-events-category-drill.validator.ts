...
import {
  EXPECTED_CATEGORY_DRILL_COLUMNS,
...
export class AlarmsEventsCategoryDrillValidator {
  validateResponse(response: AlarmsEventsCategoryDrillResponse) {
    assertZodSchema(
      AlarmsEventsCategoryDrillSuccessResponseSchema,
      response,
      "Zod Response Schema",
  ...
  validateColumns(data: AlarmsEventsCategoryDrillData) {
    expect(data.columns.map((col) => col.key)).toEqual(
      EXPECTED_CATEGORY_DRILL_COLUMNS.map((col) => col.key),
    );
    expect(data.columns.map((col) => col.header)).toEqual(
      EXPECTED_CATEGORY_DRILL_COLUMNS.map((col) => col.header),
    );
  }
  ...
  validateContext(
    ...
    expectedSlug?: string,
    expectedSeries: string = alarmsEventsCategoryDrillData.series,
  ) {
    expect(data.context.view).toBe("category-wise");
    ...
    expect(data.context.date).toBe(alarmsEventsCategoryDrillData.date);
    expect(data.context.fromDate).toBe(alarmsEventsCategoryDrillData.date);
    expect(data.context.toDate).toBe(alarmsEventsCategoryDrillData.date);
    if (expectedSlug) {
      expect(data.context.category).toBe(expectedSlug);
    }
  }

  validateUniqueColumns(data: AlarmsEventsCategoryDrillData) {
  ...
  validateUniqueRows(data: AlarmsEventsCategoryDrillData) {
  ...
  validatePaginationAndTotals(data: AlarmsEventsCategoryDrillData) {
  ...
  validateRows(data: AlarmsEventsCategoryDrillData, expectedLabel?: string) {
  ...
  validateValidationError(body: AlarmsEventsCategoryDrillResponse) {
