...
import { AlarmsEventsChartData,AlarmsEventsChartPeriod,AlarmsEventsChartResponse } from "../Mappper/alarms-events-chart.mapper";
...
export class AlarmsEventsChartValidator {
  ...
  validatePeriod(period: AlarmsEventsChartPeriod, expectedName: string) {
    ...
    expect(Object.keys(period.phases).sort()).toEqual(
      [...EXPECTED_CHART_PHASES].sort(),
    );
