...
import { AlarmsEventsPhaseDrillMapper } from "../Mapper/alarms-events-phase-drill.mapper";
  ...
  for (const testCase of alarmsEventsPhaseDrillTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      validate.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, alarmsEventsPhaseDrillData.maxResponseTime),
      );
...
authTest.describe("Alarms and Events phase drill-down — no login", () => {
  authTest(
    ...
    async ({ unauthenticatedApi }) => {
      ...
      const rawResponse = await unauthenticatedApi.get(alarmsEventsPhaseDrillData.path, {
        params: {
          category: "Power",
