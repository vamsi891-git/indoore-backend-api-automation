...
import { AlarmsEventsPriorityChartMapper } from "../Mapper/alarms-events-priority-chart.mapper";
  ...
  for (const testCase of alarmsEventsPriorityChartTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      validate.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, alarmsEventsPriorityChartData.maxResponseTime),
      );
      ...
      validate.execute("Priority", () =>
        validator.validatePriority(data, testCase.expectedPriority, testCase.expectedLabel),
      );
      validate.execute("Periods and series", () =>
        validator.validatePeriods(
          data,
          typeof testCase.params.date === "string" ? testCase.params.date : undefined,
        ),
      );
      validate.execute("No duplicate series", () => validator.validateUniqueSeries(data));
      validate.printSummary(testCase.testName, responseTime);
    });
  }
});
...
authTest.describe("Alarms and Events priority-wise chart - no login", () => {
  authTest(
    ...
    {
      tag: ["@alarms-events", "@alarms-events-priority-chart", "@negative", "@auth"],
    },
    async ({ unauthenticatedApi }) => {
      const startedAt = Date.now();
      const rawResponse = await unauthenticatedApi.get(alarmsEventsPriorityChartData.path, {
        params: {
          priority: "Priority 1",
