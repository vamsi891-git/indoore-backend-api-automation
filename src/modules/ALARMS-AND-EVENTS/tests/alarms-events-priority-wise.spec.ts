...
import { AlarmsEventsPriorityWiseMapper } from "../Mappper/alarms-events-priority-wise.mapper";
  ...
  for (const testCase of alarmsEventsPriorityWiseTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } =
        await api.getPriorityWise(testCase.params);
      ...
      validate.execute("Response Time", () =>
        assert.validateResponseTime(
          responseTime,
          alarmsEventsPriorityWiseData.maxResponseTime,
        ),
      );
      ...
      validate.execute("Expected priorities", () =>
        validator.validateExpectedPriorities(
          data,
          testCase.expectedPriorityIds,
        ),
      );
      validate.execute("No duplicate priorities", () =>
        validator.validateUniquePriorities(data),
      );
