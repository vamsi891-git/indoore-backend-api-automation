...
import {
  ...
  alarmsEventsChartTestCases,
} from "../Data/alarms-events-chart.data";
import { AlarmsEventsChartMapper } from "../Mapper/alarms-events-chart.mapper";
  ...
  for (const testCase of alarmsEventsChartTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.expectedStatus !== 200) {
        validate.execute("Error", () => validator.validateValidateError(responseBody));
      ...
      validate.execute("Periods and phases", () => validator.validatePeriods(data));
      validate.printSummary(testCase.testName, responseTime);
    });
  }
});
authTest.describe("Alarms and Events chart — no login", () => {
  authTest(
    ...
    async ({ unauthenticatedApi }) => {
      ...
      const rawResponse = await unauthenticatedApi.get(alarmsEventsChartData.path, {
        params: {
          category: "Power",
      ...
      validation.printSummary("GET /alarms-events/chart — missing token is rejected", responseTime);
    },
  );
});
