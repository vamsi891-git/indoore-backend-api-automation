...
import { AlarmsEventsCategoryWiseMapper } from "../Mapper/alarms-events-category-wise.mapper";
...
test.describe("Alarms and Events - category-wise", () => {
  ...
  for (const testCase of alarmsEventsCategoryWiseTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getCategoryWise(
        testCase.params,
      );
      ...
      validate.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, alarmsEventsCategoryWiseData.maxResponseTime),
      );
      ...
      validate.execute("Expected categories", () => validator.validateExpectedCategories(data));
      validate.execute("No duplicate categories", () => validator.validateUniqueCategories(data));
...
authTest.describe("Alarms and Events category-wise - no login", () => {
  authTest(
    ...
    {
      tag: ["@alarms-events", "@alarms-events-category-wise", "@negative", "@auth"],
    },
    async ({ unauthenticatedApi }) => {
      const startedAt = Date.now();
      const rawResponse = await unauthenticatedApi.get(alarmsEventsCategoryWiseData.path);
