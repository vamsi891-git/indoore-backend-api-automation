export interface AlarmsEventsPhaseDrillColumn {
  key: string;
  header: string;
}

export interface AlarmsEventsPhaseDrillRow {
  id: string;
  eventId: number;
  eventName: string;
...
export interface AlarmsEventsPhaseDrillData {
  columns: AlarmsEventsPhaseDrillColumn[];
  rows: AlarmsEventsPhaseDrillRow[];
  pagination: {
    page: number;
    limit: number;
    total: number;
...
export interface AlarmsEventsPhaseDrillResponse {
  success: boolean;
  data?: AlarmsEventsPhaseDrillData;
  error?: { code?: string; message?: string };
}

export class AlarmsEventsPhaseDrillMapper {
  static map(response: AlarmsEventsPhaseDrillResponse): AlarmsEventsPhaseDrillData {
    const data = response.data ?? ({} as AlarmsEventsPhaseDrillData);
    return {
