...
import { meterMasterMaxResponseTimeMs, meterMasterTestCases } from "../Data/meter-master.data";
...
test.describe("Master data — meter list", () => {
  ...
  for (const testCase of meterMasterTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getMeterMasterData(
        testCase.query,
      );
      ...
      const mapped = MeterMasterMapper.mapData(responseBody.data, testCase.query.limit ?? 20);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, meterMasterMaxResponseTimeMs),
      );
