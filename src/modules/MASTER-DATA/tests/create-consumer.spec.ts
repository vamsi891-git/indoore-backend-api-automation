...
function shouldSkipForEnv(testCase: (typeof createConsumerTestCases)[number]): boolean {
...
function needsAssignableMeter(testCase: (typeof createConsumerTestCases)[number]): boolean {
  return !METER_SCENARIO_SCENARIOS.has(testCase.scenario);
}

function needsNearestAcctId(testCase: (typeof createConsumerTestCases)[number]): boolean {
  return !["invalid_nearest_acct_id", "missing_nearest_acct_id"].includes(testCase.scenario);
}
...
test.describe.skip("Master data — add consumer", () => {
  ...
  test.beforeAll(async ({ authenticatedApi }) => {
    ...
    console.log(`[create-consumer] dtr network lookup id: ${runtime?.networkLookupId ?? "none"}`);
    console.log(`[create-consumer] existing consumer id: ${existingCid ?? "none"}`);
  });
  ...
  for (const testCase of createConsumerTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (needsAssignableMeter(testCase) && !hasCreateConsumerMeterPool()) {
        test.skip(true, "No assignable meters provisioned via add-meter for create-consumer tests");
        return;
      }
      ...
      if (testCase.scenario === "consumer_id_exists" && !hasBulkConsumerExistingCid()) {
      ...
      if (testCase.scenario === "create_success") {
        const fresh = await provisionFreshConsumerAssignableMeters(authenticatedApi, 1, {
      ...
      let { rawResponse, responseBody, responseTime } = await api.createConsumer(requestBody);

      const acceptableStatuses = testCase.acceptableStatuses ?? [testCase.expectedStatus];

      // Retry create_success on meter collision with another brand-new MSN.
      if (testCase.scenario === "create_success") {
        for (
          let attempt = 1;
          attempt <= 3 && !acceptableStatuses.includes(rawResponse.status());
          attempt += 1
        ) {
          console.warn(
          ...
          ({ rawResponse, responseBody, responseTime } = await api.createConsumer(requestBody));
        }
      }
      ...
      if (statusOk && SUCCESS_SCENARIOS.has(testCase.scenario)) {
        ...
        if (responseBody.message !== undefined) {
          validation.execute("Success Message Present", () => {
            expect(String(responseBody.message ?? "").trim().length).toBeGreaterThan(0);
          });
        }
      } else if (statusOk) {
        validation.execute("Required Fields", () =>
      ...
      if (testCase.scenario === "create_success" && mapped.isCreateSuccess) {
        ...
        validation.execute("Post Create Profile Backend Rules", () =>
          validator.validatePostCreateProfileBackendRules(profileResult.responseBody, requestBody),
        );
      }
      ...
      if (
        ...
        validation.execute("Consumer Not Persisted Status", () =>
          assert.validateStatusCode(profileResult.rawResponse, 404, profileResult.responseBody),
        );
        validation.execute("Consumer Not Persisted", () =>
          validator.validatePostCreateConsumerNotFound(profileResult.responseBody),
        );
      }
