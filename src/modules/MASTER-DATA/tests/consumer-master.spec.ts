...
test.describe("Master data — consumer list", () => {
  ...
  for (const testCase of consumerMasterTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getConsumerMasterData(
        testCase.query,
      );
      ...
      if (testCase.query.includeArchiveCounts != null) {
        params.set("includeArchiveCounts", String(testCase.query.includeArchiveCounts));
      }
      ...
      const mapped = ConsumerMasterMapper.mapData(responseBody.data, testCase.query.limit ?? 20);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, consumerMasterMaxResponseTimeMs),
      );
      ...
      validation.execute("Sl No Sequence", () => validator.validateSlNoSequence(mapped));
      ...
      validation.execute("IVRS Consistency", () => validator.validateIvrsConsistency(mapped));
      validation.execute("Meter Phases", () => validator.validateMeterPhases(mapped));
      validation.execute("Hierarchy Fields", () => validator.validateHierarchyFields(mapped));
