...
import { updateMeterMaxResponseTimeMs, updateMeterTestCases } from "../Data/update-meter.data";
...
test.describe.skip("Master data — update meter", () => {
  ...
  for (const testCase of updateMeterTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const baseCreatePayload = buildCreateMeterRequest();
      ...
      if (testCase.provisionMeter) {
        ...
        meterLookupTblRefId = created.responseBody.data!.meterLookupTblRefId;
      }

      const requestBody = testCase.buildPayload(baseCreatePayload);
      const api = new UpdateMeterApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await api.updateMeter(
        meterLookupTblRefId,
        requestBody,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, updateMeterMaxResponseTimeMs),
      );
