...
function needsAssignableMeter(testCase: (typeof createDtrTestCases)[number]): boolean {
  return !METER_SCENARIO_SCENARIOS.has(testCase.scenario);
}

function shouldSkipForEnv(testCase: (typeof createDtrTestCases)[number]): boolean {
  ...
  for (const testCase of createDtrTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (shouldSkipKnownBackendDefects() && testCase.tags.includes("@backend-defect")) {
        test.skip(true, "Known backend defect — see Bulk upload validations.txt (CREATE DTR)");
        return;
      }
      ...
      if (needsAssignableMeter(testCase) && !hasResolvedUnmappedMeters()) {
        test.skip(true, "No assignable meters provisioned via add-meter for create-dtr tests");
        return;
      }
      ...
      if (testCase.scenario === "success") {
        const fresh = await provisionFreshDtrAssignableMeters(authenticatedApi, 1, {
          maxCreateAttempts: 10,
        });
        if (!fresh[0]) {
          test.skip(true, "Could not provision a fresh assignable meter for create-dtr success");
          return;
        }
        requestBody = {
          ...requestBody,
      ...
      let { rawResponse, responseBody, responseTime } = await api.createDtr(requestBody);

      // One retry on 409 for success — usually modem/SIM collision from prior runs.
      if (testCase.scenario === "success" && rawResponse.status() === 409) {
        console.warn(`[create-dtr] success got 409 — reprovisioning meter and unique modem fields`);
        const retryMeters = await provisionFreshDtrAssignableMeters(authenticatedApi, 1, {
          maxCreateAttempts: 10,
        });
        if (retryMeters[0]) {
          requestBody = {
          ...
          ({ rawResponse, responseBody, responseTime } = await api.createDtr(requestBody));
        }
      }
      ...
      validation.execute("Status Validation", () => {
        ...
        const statuses = testCase.acceptableStatuses ?? [testCase.expectedStatus];
