...
export function collectMasterDataDataQualityFindings(
  kind: string,
...
export async function logMasterDataDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectMasterDataDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `MASTER-DATA ${kind} DQ`);
  return report;
}
