...
export function collectConsumptionDataQualityFindings(
  kind: string,
...
export async function logConsumptionDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectConsumptionDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `CONSUMPTION ${kind} DQ`);
  return report;
}
