...
test.describe("Consumption API — Negative", () => {
  for (const testCase of allNegativeCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { response } = await getConsumptionWithRetry(authenticatedApi, testCase.path, {
      ...
      validation.execute("Error envelope", () =>
        ConsumptionCommonValidator.validateErrorEnvelope(responseBody, testCase.expectedCodes),
      );
