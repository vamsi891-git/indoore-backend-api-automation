...
test.describe("Hourly consumption list", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { page, limit, fromDate, toDate, month, year, maxResponseTime } = hourlyConsumptionData;
      ...
      if (isOk && mapped.items.length > 0) {
        ...
        validation.execute("Hour Buckets", () => validator.validateHourBuckets(mapped.items));
