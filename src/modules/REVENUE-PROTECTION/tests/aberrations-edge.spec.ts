...
test.describe("Revenue Protection — Aberrations Edge", () => {
  ...
  test(
    "Page far beyond total returns empty rows with consistent pagination",
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getAberrationSummary({
  ...
  test(
    "Single-page result: total count equals returned records",
    { tag: ["@revenue-protection", "@aberrations", "@edge"] },
    async ({ authenticatedApi }) => {
      const api = new AberrationsApi(authenticatedApi);
      const validation = new ApiValidationHelper();
