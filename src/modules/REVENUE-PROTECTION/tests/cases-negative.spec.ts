...
test.describe("Revenue Protection — Cases Negative", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const baselineBody = (await baseline.json().catch(() => ({}))) as RevenueErrorBody;
      const baselineMapped = CasesMapper.mapData(baselineBody.data as CasesRawData);
      ...
      validation.execute("Cross-org scope preserved", () => {
        ...
        expect(body.success).toBeTruthy();
        const mapped = CasesMapper.mapData(body.data as CasesRawData);
        expect(mapped.pagination.total).toEqual(baselineMapped.pagination.total);
      });
