...
import {
  ...
  revenueProtectionPaths, // ADD `atrZone` to this map — see note below
...
authTest.describe("Revenue Protection — ATR Zone Auth Negative", () => {
  ...
  authTest(
    "IND-RPT-ATZ-NEG-011 — ATR Zone rejects missing auth",
  ...
  authTest(
    "IND-RPT-ATZ-NEG-012 — ATR Zone rejects invalid bearer token",
    { tag: ["@revenue-protection", "@atr-zone", "@negative", "@auth"] },
    async ({ unauthenticatedApi }) => {
  ...
  authTest(
    "IND-RPT-ATZ-NEG-013 — ATR Zone rejects disallowed HTTP methods",
    { tag: ["@revenue-protection", "@atr-zone", "@negative", "@auth"] },
    async ({ unauthenticatedApi }) => {
