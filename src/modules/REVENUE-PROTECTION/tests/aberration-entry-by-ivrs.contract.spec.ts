...
import { assertContractSnapshot } from "../../../extras/contract/contract-snapshot.helper";
...
test.describe("Revenue Protection — Aberration Entry By IVRS Contract Snapshot", () => {
  ...
  test(
    "IND-RPT-ABE-IVRS-CONTRACT-001 — By-IVRS PATCH success response keys contract",
    {
      tag: ["@revenue-protection", "@aberration-entry-by-ivrs", "@contract-snapshot"],
    },
    async () => {
      await applyAllureTestCaseId("IND-RPT-ABE-IVRS-CONTRACT-001");
      await assertContractSnapshot("revenue-protection/aberration-entry-by-ivrs-response", {
        httpMethod: "PATCH",
