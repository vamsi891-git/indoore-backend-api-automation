...
const casesUrl = `${revenueProtectionPaths.cases}?${buildCasesQueryString(
  casesDefaultQuery,
)}`;
...
authTest.describe("Revenue Protection — Cases Auth Negative", () => {
  ...
  authTest(
    "IND-RPT-NEG-011 — Cases rejects missing auth",
  ...
  authTest(
    "IND-RPT-NEG-012 — Cases rejects invalid bearer token",
    { tag: ["@revenue-protection", "@cases", "@negative", "@auth"] },
    async ({ unauthenticatedApi }) => {
  ...
  authTest(
    "IND-RPT-NEG-013 — Cases rejects disallowed HTTP methods",
    { tag: ["@revenue-protection", "@cases", "@negative", "@auth"] },
    async ({ unauthenticatedApi }) => {
