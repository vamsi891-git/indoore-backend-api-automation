...
test.describe("Revenue Protection — Aberration Entry By IVRS (PATCH)", () => {
  ...
  for (const testCase of aberrationEntryByIvrsTestCases) {
    test(testCase.testName, { tag: [...testCase.tags] }, async ({ authenticatedApi, obs }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.patchAberrationEntryByIvrs(
        ivrsNo,
        payload,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, aberrationEntryByIvrsMaxResponseTimeMs),
      );
      ...
      validation.execute("IVRS Echo", () => validator.validateIvrsEcho(mapped, ivrsNo));
