...
import { aberrationEntryEenltmtTestCases } from "../Data/aberration-entry-eenltmt.data";
...
test.describe("Revenue Protection — Aberration Entry EENLTMT API", () => {
  test.describe.configure({
    retries: 1,
    mode: "serial",
  });
  test.setTimeout(REVENUE_PROTECTION_TEST_TIMEOUT_MS);
  for (const testCase of aberrationEntryEenltmtTestCases) {
    test(
      testCase.testName,
      { tag: [...testCase.tags] },
