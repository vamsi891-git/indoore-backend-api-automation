...
import { DaywiseBillingMapper, type DaywiseBillingData } from "../Mapper/daywisebilling.mapper";
...
function runFilledPageRules(
  ...
  validation.execute("Meter details", () => validator.validateMeterDetails(mapped));
  validation.execute("Consumer", () => validator.validateConsumerData(mapped));
  validation.execute("Daily kWh", () => validator.validateDailyKwhValues(mapped));
  validation.execute("Daily reading trend", () => validator.validateDailyReadingTrend(mapped));
  validation.execute("Days in month", () => validator.validateDaysInMonth(mapped));
  ...
  validation.execute("Null-safe hierarchy", () => validator.validateNullSafeFields(mapped));
  ...
  for (const testCase of daywiseBillingTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        ...
        validation.execute("Has more", () => validator.validateHasMoreFlag(mapped));
        runFilledPageRules(
          validator,
          validation,
          mapped,
      ...
      const { rawResponse, responseBody, responseTime } = await api.getDaywiseBillingData(query);
      ...
      validation.execute("Data exists", () => validator.validateDataExists(mapped));
      validation.execute("Pagination", () => validator.validatePagination(mapped, includeTotal));
      validation.execute("Month year", () =>
        validator.validateMonthYear(mapped, query.month ?? 10, query.year ?? 2025),
      );
      validation.execute("Has more", () => validator.validateHasMoreFlag(mapped));

      if (testCase.scenario === "dev_meter_filter" && mapped.items.length === 0) {
        test.skip(true, `No matching meter ${daywiseKnownMeter} in Oct 2025 daywise billing`);
      }
