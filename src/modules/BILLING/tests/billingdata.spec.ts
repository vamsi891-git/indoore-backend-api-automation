...
function runFilledPageRules(
  ...
  validation.execute("Billing Items", () =>
    validator.validateBillingItems(mapped),
  );
  validation.execute("Sparse hierarchy", () =>
    validator.validateSparseHierarchyAllowed(mapped),
  );
  validation.execute("Power Factor", () =>
    validator.validatePowerFactor(mapped),
  );
  validation.execute("Energy Calculation", () =>
    validator.validateEnergyCalculation(mapped),
  );
  validation.execute("KVAH Calculation", () =>
    validator.validateKvahCalculation(mapped),
  );
  ...
  validation.execute("Export Energy", () =>
    validator.validateExportEnergy(mapped),
  );
  validation.execute("MD occurrence time", () =>
    validator.validateMdOccurrenceTimes(mapped),
  );
    ...
    test(
      ...
      async ({ authenticatedApi }) => {
        ...
        if (testCase.isContractFixture) {
          ...
          validation.execute("Grid meta", () =>
            validator.validateGridMeta(mapped),
          );
          runFilledPageRules(
            validator,
            validation,
        ...
        const { rawResponse, responseBody, responseTime } =
          await api.getBillingData(query);
        ...
        validation.execute("Billing Data Exists", () =>
          validator.validateBillingDataExists(mapped),
        );
        ...
        if (
          ...
          test.skip(
            true,
            `No matching meter ${billingDataKnownMeter} in Oct 2025 billing data`,
          );
        }
