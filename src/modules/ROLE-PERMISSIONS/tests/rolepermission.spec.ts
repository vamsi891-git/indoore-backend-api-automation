...
test.describe.skip("Role Permission CRUD Flow", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      // UPDATE ROLE
      // =====================================
      const updateRoleResponse = await roleApi.updateRole(
      ...
      const rolePermissions = RolePermissionMapper.mapRolePermissions(
        getPermissionsResponse.responseBody,
      );
      ...
      // ASSIGN PERMISSIONS
      // =====================================
      const assignPermissionPayload = RolePermissionMapper.buildAssignPermissionPayload(
        rolePermissions.modules,
      );
      const assignPermissionResponse = await roleApi.assignPermissions(
        RolePermissionData.roleId,
      ...
      // VERIFY ASSIGNMENT
      // =====================================
      const verifyResponse = await roleApi.getRolePermissions(RolePermissionData.roleId);
      const verifyPermissions = RolePermissionMapper.mapRolePermissions(
      ...
      // TOGGLE 2FA
      // =====================================
      const toggle2FAResponse = await roleApi.toggle2FA(RolePermissionData.toggle2FAPayload);
      ...
      if (toggleStatus === 404) {
        ...
        const toggle2FAData = RolePermissionMapper.mapToggle2FA(toggle2FAResponse.responseBody);
        validation.execute("Validate Toggle 2FA", () => validator.validateToggle2FA(toggle2FAData));
      }
      // =====================================
      // STEP 8
      // DELETE ROLE
      // =====================================
      const deleteResponse = await roleApi.deleteRole(RolePermissionData.roleId);
