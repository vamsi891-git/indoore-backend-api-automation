...
test.describe.skip("Role Permission — Advanced", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const targetModule =
        RolePermissionMapper.findModule(modules, RolePermissionData.preferredModuleKey) ??
        modules[0];
      if (!targetModule) {
      ...
      validation.execute("Module disabled in catalog", () =>
        validator.validateModuleEnabledState(disabledModules, targetModule!.moduleId, false),
      );
      ...
      validation.execute("Re-enable module status", () =>
        assert.validateStatusCode(enableResponse.rawResponse, 200, enableResponse.responseBody),
      );
      ...
      validation.execute("Module re-enabled in catalog", () =>
        validator.validateModuleEnabledState(enabledModules, targetModule!.moduleId, true),
      );
      ...
      validation.printSummary(
        ...
        created.responseTime + disableResponse.responseTime + enableResponse.responseTime,
      );
    },
  );
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const requires = rulesResponse.responseBody.data.requires as Record<string, string[]>;
      ...
      const catalog = await roleApi.getRolePermissions(roleId);
      const modules = RolePermissionMapper.mapRolePermissions(catalog.responseBody).modules;
      const assignCase = RolePermissionMapper.pickDependencyAssignCase(requires, modules);
      ...
      validation.execute("Assign child permission status", () =>
        assert.validateStatusCode(assignResponse.rawResponse, 200, assignResponse.responseBody),
      );

      const verifyResponse = await roleApi.getRolePermissions(roleId);
      const grantedKeys = RolePermissionMapper.collectGrantedKeys(
        RolePermissionMapper.mapRolePermissions(verifyResponse.responseBody).modules,
      );
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const catalog = await roleApi.getRolePermissions(roleId);
      const modules = RolePermissionMapper.mapRolePermissions(catalog.responseBody).modules;
      const replaceCase = RolePermissionMapper.pickReplacePermissionKeys(modules, requires);
      if (!replaceCase) {
        await roleApi.deleteRole(roleId);
      ...
      validation.execute("First assign status", () =>
        assert.validateStatusCode(firstAssign.rawResponse, 200, firstAssign.responseBody),
      );
      ...
      validation.execute("First keys granted", () =>
        validator.validateAssignedPermissions(modulesAfterFirst, replaceCase!.firstKeys),
      );
      ...
      validation.execute("Second assign status", () =>
        assert.validateStatusCode(secondAssign.rawResponse, 200, secondAssign.responseBody),
      );
      ...
      validation.execute("Second assign replaces first key", () => {
        validator.validateAssignedPermissions(modulesAfterSecond, replaceCase!.secondKeys);
        validator.validateKeyNotGranted(modulesAfterSecond, replaceCase!.firstKeys[0]);
      });
      ...
      validation.execute("Empty array revokes all permissions", () =>
        validator.validateNoGrantedPermissions(
          RolePermissionMapper.mapRolePermissions(afterRevoke.responseBody).modules,
        ),
      );
      await roleApi.deleteRole(roleId);
      validation.printSummary(
