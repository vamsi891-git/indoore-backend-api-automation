...
test.describe.skip("Role Permission — Negative", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await roleApi.createRole(
        RolePermissionData.invalidRoleNameUppercase,
      );
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const roleApi = new RolePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await roleApi.createRole(
        RolePermissionData.invalidRoleNameShort,
      );
      validation.execute("Status (validation error)", () =>
        assert.validateStatusCode(rawResponse, 400, responseBody),
      );
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await roleApi.updateRole(roleId, {});
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const validator = new RolePermissionValidator();
      const roleApi = new RolePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await roleApi.updateRole(
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const created = await roleApi.createRole(payload);
      const roleId = RolePermissionMapper.mapRole(created.responseBody).id;
      const { rawResponse, responseBody, responseTime } = await roleApi.assignPermissions(
        roleId,
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const protectedRole = roles.find((role) => validator.isProtectedRoleName(role.name));
      if (!protectedRole) {
        test.skip(true, "No protected role found in catalog");
      }
      const { rawResponse, responseBody, responseTime } = await roleApi.deleteRole(
        protectedRole!.id,
      );
      const status = rawResponse.status();
      if (BackendResponse.shouldSkipServerFailure(status, "Delete Protected Role", responseBody)) {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const validator = new RolePermissionValidator();
      const roleApi = new RolePermissionApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await roleApi.setRoleModuleEnabled(
        RolePermissionData.unknownResourceId,
        1,
