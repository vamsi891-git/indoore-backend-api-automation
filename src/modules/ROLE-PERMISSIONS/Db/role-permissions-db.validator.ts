...
export function collectRolePermissionsDataQualityFindings(
  kind: string,
...
export async function logRolePermissionsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectRolePermissionsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `ROLE-PERMISSIONS ${kind} DQ`);
  return report;
}
