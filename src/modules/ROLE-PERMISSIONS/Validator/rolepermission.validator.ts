...
import {
  Role,
  Module,
  RolePermissionDataResponse,
  Toggle2FAResponse,
} from "../Mapper/rolepermission.mapper";
export class RolePermissionValidator {
  ...
  validateMyPermissionsResponse(body: { success?: boolean; data?: { permissions?: string[] } }) {
    expect(body.success).toBe(true);
  ...
  validateModuleEnabledState(modules: Module[], moduleId: number, expectedEnabled: boolean) {
  ...
  validateExpandedDependencies(grantedKeys: string[], assignedKey: string, requiredKeys: string[]) {
  ...
  isProtectedRoleName(name: string): boolean {
    ...
    return ["admin", "super_admin", "superadmin", "manager", "viewer", "user"].includes(normalized);
  }
}
