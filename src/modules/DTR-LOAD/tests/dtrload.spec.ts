...
import { DtrLoadResponseSchema, type ParsedDtrLoadResponse } from "../schemas/dtrload.schemas";
...
function runFilledPageRules(
  ...
  validation.execute("Unique meters", () => validator.validateUniqueMeters(mapped));
  validation.execute("Shared feeder allowed", () => validator.validateSharedKeysAllowed(mapped));
  validation.execute("MSN matches serial", () => validator.validateMsnMatchesSerial(mapped));
  if (
    type === "hourly_actual_load" ||
    type === "hourly_load_percentage" ||
    type === "consumption"
  ) {
    validation.execute("Hourly values", () => validator.validateHourlyNonNegative(mapped));
  }
  if (type === "hourly_actual_load") {
    validation.execute("Hourly totals", () => validator.validateHourlyTotals(mapped));
  }
  if (type === "loading") {
    validation.execute("Load percent", () => validator.validateLoadingPercent(mapped));
  }
  if (type === "unbalance_loading") {
    validation.execute("Phase currents", () => validator.validateUnbalancePhases(mapped));
  }
  if (type === "load_summary") {
    validation.execute("Summary loads", () => validator.validateSummaryLoads(mapped));
  }
  if (type === "consumption") {
    validation.execute("Consumption unit", () => validator.validateConsumptionUnit(mapped, query));
  }
}
  ...
  for (const testCase of dtrLoadTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        ...
        const type =
          testCase.scenario === "contract_consumption" ? "consumption" : "hourly_actual_load";
        validation.execute("Required Fields", () =>
          assert.validateRequiredFields(fixtureBody, ["success", "data"]),
        );
        validation.execute("Columns", () => validator.validateColumns(mapped.columns, type));
      ...
      const { rawResponse, responseBody, responseTime } = await api.getDtrLoad(query);
      ...
      skipIfDtrLoadInternalError(rawResponse.status(), responseBody, "/indore/dtr-load");
      ...
      validation.execute("No Data Scenario", () => validator.validateNoData(mapped));
