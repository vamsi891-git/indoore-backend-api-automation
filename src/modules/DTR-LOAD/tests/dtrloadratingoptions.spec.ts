...
function runItemRules(
  ...
  validation.execute("Unique values", () => validator.validateUniqueValues(mapped));
  validation.execute("Ascending order", () => validator.validateAscendingOrder(mapped));
}
    ...
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        ...
        validation.execute("Zod Response Schema", () => {
          const result = DtrLoadRatingOptionsResponseSchema.safeParse(dtrLoadRatingOptionsFixture);
          expect(
            result.success,
            result.success
        ...
        validation.execute("Required Fields", () =>
          assert.validateRequiredFields(dtrLoadRatingOptionsFixture, ["success", "data"]),
        );
      ...
      const { rawResponse, responseBody, responseTime } = await api.getRatingOptions(query);
      ...
      let parsed!: ParsedDtrLoadRatingOptionsResponse;
      validation.execute("Zod Response Schema", () => {
        const result = DtrLoadRatingOptionsResponseSchema.safeParse(responseBody);
        expect(
          result.success,
          result.success
            ? "Zod validation passed"
      ...
      runItemRules(
        ...
        validation,
        mapped,
        testCase.scenario === "live_oct" || testCase.scenario === "live_unknown_query",
      );
