...
test.describe("Audit Log Export API",() => {
        ...
        test("Validate Audit Log Export ASC API",
            ...
            async ({ authenticatedApi }) => {
                ...
                validation.execute("File Not Empty",() =>
                        new AuditLogExportValidator().validateFileNotEmpty(csvContent)
                );
                validation.execute("Header Validation",() =>
                        new AuditLogExportValidator().validateHeaders(csvContent)
                );
                const rows =CsvParser.parseAuditLogCsv(csvContent);
                const validator = new AuditLogExportValidator();
                validation.execute( "Row Count Validation", () =>
                        validator.validateRowCount(rows,AuditLogExportTestData.limit)
                );
                validation.execute("Audit Rows Validation",() =>
                        validator.validateAuditRows(rows)
                );
                validation.execute("UUID Validation",() =>
                        validator.validateUUIDs(rows)
                );
                validation.execute("Email Validation",() =>
                        validator.validateEmails(rows)
                );
                validation.execute("Role Validation",() =>
                        validator.validateRoles(rows)
                );
                validation.execute("Full Name Validation",() =>
                        validator.validateFullNames(rows)
                );
                validation.execute("Action Validation",() =>
                        validator.validateActions(rows)
                );
                validation.execute("Created At Validation",() =>
                        validator.validateCreatedAt(rows)
                );
                validation.execute("IP Address Validation",() =>
                        validator.validateIpAddresses(rows)
                );
                validation.execute("Details Validation",() =>
                        validator.validateDetails(rows)
                );
                validation.execute("Duplicate ID Validation",() =>
                        validator.validateDuplicateIds(rows)
                );
                validation.execute("Target Consistency Validation",() =>
                ...
                validation.printSummary("Audit Log Export ASC API",responseTime);
            }
        );
        test("Validate Audit Log Export DESC API",
            {
                tag: [
                    "@smoke",
                    "@audit",
            ...
            async ({ authenticatedApi }) => {
                ...
                validation.printSummary("Audit Log Export DESC API",responseTime);
            }
        );
    }
);