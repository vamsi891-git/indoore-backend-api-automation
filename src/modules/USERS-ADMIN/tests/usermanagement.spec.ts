...
test.describe("User Management Flow", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      validation.execute("Validate Users Exist", () => validator.validateUsers(users));
      validation.execute("Validate User Structure", () => validator.validateUserStructure(users));
      validation.execute("Validate User Types", () => validator.validateUserTypes(users));
      validation.execute("Validate Email Format", () => validator.validateEmailFormat(users));
      validation.execute("Validate Duplicate Users", () => validator.validateDuplicateUsers(users));
      validation.execute("Validate Role Rules", () => validator.validateRoleRules(users));
      validation.execute("Validate Status Rules", () => validator.validateStatusRules(users));
      validation.execute("Validate Session Rules", () => validator.validateSessionRules(users));
      validation.execute("Validate Date Rules", () => validator.validateDates(users));
      validation.execute("Validate Null Handling", () => validator.validateNullHandling(users));
      validation.execute("Validate NaN Handling", () => validator.validateNaNValues(users));
      const selectedUser =
        users.find(
          (user: User) =>
            user.id === UserDevicesTestConfig.deviceTestUserId && !isAutomationAccount(user),
        ) ??
        users.find(
          (user: User) => user.role.toLowerCase() === "manager" && !isAutomationAccount(user),
        ) ??
      ...
      validation.execute("Validate Created By", () => validator.validateCreatedBy(user));
      validation.execute("Validate Invited By", () => validator.validateInvitedBy(user));
      validation.execute("Validate Two Factor Rules", () => validator.validateTwoFactorRules(user));
      UserManagementData.updateUserPayload = UserManagementData.buildUpdateUserPayload(user);
      await PerformanceTracker.track(
      ...
      // UPDATE STATUS
      // =====================================
      const statusResponse = await userApi.updateUserStatus(
        UserManagementData.userId,
      ...
      validation.execute("Verify Updated Status", () =>
        validator.validateStatusUpdate(
          verifiedStatusUser,
          UserManagementData.updateStatusPayload.status,
        ),
      );
      // =====================================
      // FINAL ASSERTIONS
