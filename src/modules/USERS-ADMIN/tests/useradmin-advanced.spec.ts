...
test.describe("User Admin — Advanced", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const deviceToDelete = UserManagementMapper.pickDeletableDevice(mapped.devices);
      ...
      const verifyMapped = UserManagementMapper.mapDevices(verifyResponse.responseBody);
      validation.execute("Verify Device Removed From Catalog", () => {
        expect(
          verifyMapped.devices.find((device) => device.id === deviceToDelete!.id),
        ).toBeUndefined();
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const verifiedUser = UserManagementMapper.mapUser(verifyResponse.responseBody);
  ...
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const validator = new UserManagementValidator();
      const userApi = new UserManagementApi(authenticatedApi);
      const response = await userApi.getAuditLogs(UserManagementData.auditLogActionFilter);
