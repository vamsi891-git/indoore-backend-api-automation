...
test.describe("User Admin — Advanced", () => {
    ...
    test("DELETE /users/:id/devices/:deviceId — revoke device and verify catalog",
        ...
        async ({ authenticatedApi }) => {
            ...
            const deleteResponse = await userApi.deleteDevice(userId,deviceToDelete!.id,);
            ...
            validation.execute("Verify Device Removed From Catalog", () => {
                expect(verifyMapped.devices.find((device) => device.id === deviceToDelete!.id,),).toBeUndefined();
                expect(verifyMapped.devices.length).toBeLessThan(mapped.devices.length,);
            });
            validation.printSummary("Delete User Device",listResponse.responseTime + deleteResponse.responseTime,);
        },
    );
    test("PATCH /users/:id/status — suspend then restore active",
        { tag: ["@users-admin"] },
        async ({ authenticatedApi }) => {
            const assert = new AssertionEngine();
            const validation = new ValidationEngine();
            ...
            validation.execute("Suspend Status Code", () =>
                assert.validateStatusCode(suspendResponse.rawResponse,200,suspendResponse.responseBody,),);
            const suspendedUser = UserManagementMapper.mapUser(suspendResponse.responseBody,);
            validation.execute("Validate Suspended Status", () =>
                validator.validateStatusUpdate(suspendedUser,UserManagementData.suspendStatusPayload.status,),
            );
            ...
            validation.execute("Restore Active Status Code", () =>
                assert.validateStatusCode(restoreResponse.rawResponse,200,restoreResponse.responseBody,),
            );
            const restoredUser = UserManagementMapper.mapUser(restoreResponse.responseBody,);
            validation.execute("Validate Restored Active Status", () =>
                validator.validateStatusUpdate(restoredUser, UserManagementData.activeStatusPayload.status,),
            );
            ...
            validation.execute("Verify Status Persisted", () =>
                validator.validateStatusUpdate(verifiedUser,UserManagementData.activeStatusPayload.status,),
            );
            validation.printSummary("User Status Toggle",suspendResponse.responseTime + restoreResponse.responseTime,);
        },
    );
    test("GET /users/audit-logs — filter by action",
        { tag: ["@users-admin"] },
        async ({ authenticatedApi }) => {
            const assert = new AssertionEngine();
            const validation = new ValidationEngine();
            ...
            validation.execute("Filtered audit logs status", () =>
                assert.validateStatusCode(response.rawResponse,200,response.responseBody,),
            );
            const mapped = UserManagementMapper.mapAuditLogs(response.responseBody,);
