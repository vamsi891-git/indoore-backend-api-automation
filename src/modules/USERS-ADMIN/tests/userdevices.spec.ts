...
test.describe("User Devices Flow", () => {
  ...
  test("Validate User Devices", { tag: ["@users-admin"] }, async ({ authenticatedApi }) => {
    ...
    validation.execute("Get Devices Response Time", () =>
      assert.validateResponseTime(devicesResponse.responseTime, UserManagementData.maxResponseTime),
    );
    const mapped = UserManagementMapper.mapDevices(devicesResponse.responseBody);
    ...
    if (mapped.devices.length === 0) {
      ...
      validation.printSummary("User Devices Module (read-only)", devicesResponse.responseTime);
    ...
    validation.printSummary(
      "User Devices Module",
      devicesResponse.responseTime + deleteResponse.responseTime + verifyResponse.responseTime,
    );
    await PerformanceTracker.track(
      devicesResponse.rawResponse,
      "Get Devices",
