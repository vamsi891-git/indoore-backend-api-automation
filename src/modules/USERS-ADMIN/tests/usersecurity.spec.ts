...
test.describe("User Security Flow", () => {
  ...
  test("Validate Force Logout", { tag: ["@users-admin"] }, async ({ authenticatedApi }) => {
    ...
    const selectedUser =
      usersData.users.find(
        (u: User) => u.role.toLowerCase() === "manager" && !isAutomationAccount(u),
      ) ||
    ...
    validation.execute("Validate Force Logout", () => validator.validateForceLogout(logoutResult));
