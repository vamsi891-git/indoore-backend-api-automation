...
export function collectUsersAdminDataQualityFindings(
  kind: string,
...
export async function logUsersAdminDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectUsersAdminDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `USERS-ADMIN ${kind} DQ`);
  return report;
}
