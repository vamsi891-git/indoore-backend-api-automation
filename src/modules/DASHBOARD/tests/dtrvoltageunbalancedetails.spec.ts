...
const VOLTAGE_UNBALANCE_DETAILS_PATH = "/indore/dashboard/dtr/voltage-unbalance-details";
  ...
  for (const testCase of dtrVoltageUnbalanceDetailsTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrVoltageUnbalanceDetailsContractBody(testCase.scenario);
        if (!fixtureBody) {
          test.skip(true, "Missing DTR voltage-unbalance-details contract body");
          return;
        }

        const mapped = DtrVoltageUnbalanceDetailsMapper.map(fixtureBody);
      ...
      const query = resolveDtrVoltageUnbalanceDetailsQuery(testCase.scenario);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrVoltageUnbalanceDetailsMaxResponseTimeMs),
      );
      ...
      const mapped = DtrVoltageUnbalanceDetailsMapper.map(responseBody);
      validation.execute("Response Envelope", () =>
        validator.validateResponseEnvelope(responseBody),
      );
      validation.execute("DTR Voltage Unbalance Details Scenario", () =>
...
authTest.describe("DTR voltage unbalance list — cannot open without a valid login", () => {
  ...
  for (const authCase of dtrUnbalanceAuthNegativeCases) {
    authTest(
      `${authCase.testName}`,
      {
        tag: [...authCase.tags, "@dtr-voltage-unbalance-details"],
      },
      async ({ unauthenticatedApi }) => {
        ...
        const rawResponse = await unauthenticatedApi.get(VOLTAGE_UNBALANCE_DETAILS_PATH, {
