  ...
  for (const testCase of consumerOemDistributionTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveConsumerOemDistributionContractBody(testCase.scenario);
        if (!fixtureBody) {
          test.skip(true, "Missing consumer oem-distribution contract body");
          return;
        }
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, consumerOemDistributionMaxResponseTimeMs),
      );
  ...
  for (const authCase of dtrUnbalanceAuthNegativeCases) {
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        const rawResponse = await unauthenticatedApi.get(OEM_DISTRIBUTION_PATH, {
        ...
        if (
          ...
          authTest.skip(true, `Rate limited (429) on ${OEM_DISTRIBUTION_PATH} — retry later`);
          return;
        }
