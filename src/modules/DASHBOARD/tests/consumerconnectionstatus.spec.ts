    ...
    test(
      ...
      async ({ authenticatedApi }) => {
        ...
        if (testCase.isContractFixture) {
          const fixtureBody = resolveConsumerConnectionStatusContractBody(
            testCase.scenario,
          );
          if (!fixtureBody) {
            test.skip(
              true,
              "Missing consumer connection-status contract body",
            );
            return;
          }
    ...
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        if (
          ...
          authTest.skip(
            true,
            `Rate limited (429) on ${CONNECTION_STATUS_PATH} — retry later`,
          );
          return;
        }
