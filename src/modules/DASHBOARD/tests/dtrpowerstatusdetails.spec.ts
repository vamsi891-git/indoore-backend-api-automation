...
const POWER_STATUS_DETAILS_PATH =
            ...
            async ({ authenticatedApi }) => {
                ...
                if (testCase.isContractFixture) {
                    ...
                    if (!fixtureBody) {
                        test.skip(
                            true,
                            "Missing DTR power-status-details contract body",
                        );
                        return;
                    }

                    const mapped =
                        DtrPowerStatusDetailsMapper.map(fixtureBody);
                ...
                const query = resolveDtrPowerStatusDetailsQuery(
                    testCase.scenario,
                );

                const { rawResponse, responseBody, responseTime } =
                    await api.getDtrPowerStatusDetails(query);
        ...
        authTest(
            ...
            async ({ unauthenticatedApi }) => {
                ...
                if (
                    ...
                    authTest.skip(
                        true,
                        `Rate limited (429) on ${POWER_STATUS_DETAILS_PATH} — retry later`,
                    );
                    return;
                }
