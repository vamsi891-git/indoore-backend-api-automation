  ...
  for (const testCase of dtrConsumptionTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrConsumptionContractBody(testCase.scenario);
        if (!fixtureBody) {
      ...
      const expectedPeriod = resolveDtrConsumptionExpectedPeriod(testCase.scenario);
      ...
      const { rawResponse, responseBody, responseTime } = await api.getDtrConsumption(query);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrConsumptionMaxResponseTimeMs),
      );
      ...
      if (testCase.scenario === "invalid_period") {
        validation.execute("Invalid Period Error", () =>
          validator.validateInvalidPeriodError(responseBody as DtrConsumptionErrorResponse),
        );
