...
const CONSUMPTION_DETAILS_PATH = "/indore/dashboard/dtr/consumption-details";
  ...
  for (const testCase of dtrConsumptionDetailsTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrConsumptionDetailsContractBody(testCase.scenario);
        if (!fixtureBody) {
          test.skip(true, "Missing DTR consumption-details contract body");
          return;
        }

        const mapped = DtrConsumptionDetailsMapper.map(fixtureBody);
      ...
      const query = resolveDtrConsumptionDetailsQuery(testCase.scenario);

      const { rawResponse, responseBody, responseTime } = await api.getDtrConsumptionDetails(query);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrConsumptionDetailsMaxResponseTimeMs),
      );
      ...
      const mapped = DtrConsumptionDetailsMapper.map(responseBody);
      ...
      validation.execute("Non-empty", () =>
        assert.assertPrimaryRowsIfExpected(
          testCase.nonEmptyExpected,
          mapped.rows,
          "DTR consumption details rows",
        ),
      );
  ...
  for (const authCase of dtrUnbalanceAuthNegativeCases) {
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        const rawResponse = await unauthenticatedApi.get(CONSUMPTION_DETAILS_PATH, {
        ...
        if (
          ...
          authTest.skip(true, `Rate limited (429) on ${CONSUMPTION_DETAILS_PATH} — retry later`);
          return;
        }
        ...
        validation.execute("Auth Error Envelope", () => {
          if (authCase.expectedErrorCode === dtrUnbalanceAccessTokenInvalidCode) {
            ...
            validator.validateUnauthorizedError(responseBody as DtrConsumptionDetailsErrorResponse);
          } else {
