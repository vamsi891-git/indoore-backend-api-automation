  ...
  for (const testCase of dtrVoltageUnbalanceTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrVoltageUnbalanceContractBody(testCase.scenario);
        if (!fixtureBody) {
          test.skip(true, "Missing DTR voltage-unbalance contract body");
          return;
        }
      ...
      const { rawResponse, responseBody, responseTime } = await api.getDtrVoltageUnbalance(query);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrVoltageUnbalanceMaxResponseTimeMs),
      );
...
authTest.describe("DTR voltage unbalance — cannot open without a valid login", () => {
  ...
  for (const authCase of dtrUnbalanceAuthNegativeCases) {
    authTest(
      `${authCase.testName}`,
      {
        tag: [...authCase.tags, "@dtr-voltage-unbalance"],
      },
      async ({ unauthenticatedApi }) => {
        ...
        const rawResponse = await unauthenticatedApi.get(VOLTAGE_UNBALANCE_PATH, {
          headers: authCase.headers,
        });
        if (
          BackendResponse.shouldSkipRateLimit(
            rawResponse.status(),
            `DTR voltage-unbalance ${authCase.testName}`,
          )
        ) {
          authTest.skip(true, `Rate limited (429) on ${VOLTAGE_UNBALANCE_PATH} — retry later`);
          return;
        }
        ...
        validation.execute("Auth Error Envelope", () => {
          if (authCase.expectedErrorCode === dtrUnbalanceAccessTokenInvalidCode) {
            ...
            validator.validateUnauthorizedError(responseBody as DtrVoltageUnbalanceErrorResponse);
          } else {
