...
const COMMUNICATION_DETAILS_PATH =
            ...
            async ({ authenticatedApi }) => {
                ...
                if (testCase.isContractFixture) {
                    ...
                    if (!fixtureBody) {
                        test.skip(
                            true,
                            "Missing DTR communication-details contract body",
                        );
                        return;
                    }

                    const mapped =
                        DtrCommunicationDetailsMapper.map(fixtureBody);
                ...
                const query = resolveDtrCommunicationDetailsQuery(
                    testCase.scenario,
                );
                ...
                const mapped =
                    DtrCommunicationDetailsMapper.map(responseBody);
        ...
        authTest(
            ...
            async ({ unauthenticatedApi }) => {
                ...
                if (
                    ...
                    authTest.skip(
                        true,
                        `Rate limited (429) on ${COMMUNICATION_DETAILS_PATH} — retry later`,
                    );
                    return;
                }
