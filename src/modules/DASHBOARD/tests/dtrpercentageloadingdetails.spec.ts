...
const PERCENTAGE_LOADING_DETAILS_PATH =
            ...
            async ({ authenticatedApi }) => {
                ...
                if (testCase.isContractFixture) {
                    ...
                    if (!fixtureBody) {
                        test.skip(
                            true,
                            "Missing DTR percentage-loading-details contract body",
                        );
                        return;
                    }

                    const mapped =
                        DtrPercentageLoadingDetailsMapper.map(fixtureBody);
                ...
                const api = new DtrPercentageLoadingDetailsApi(
                    authenticatedApi,
                );
                const query = resolveDtrPercentageLoadingDetailsQuery(
                    testCase.scenario,
                );
                ...
                const mapped =
                    DtrPercentageLoadingDetailsMapper.map(responseBody);
                validation.execute("Response Envelope", () =>
                    validator.validateResponseEnvelope(responseBody),
                );
                validation.execute(
                    "DTR Percentage Loading Details Scenario",
                    () =>
...
authTest.describe(
    "DTR loading list — cannot open without a valid login",
    () => {
        ...
        for (const authCase of dtrUnbalanceAuthNegativeCases) {
            authTest(
                ...
                async ({ unauthenticatedApi }) => {
                    const validator =
                        new DtrPercentageLoadingDetailsValidator();
                    const assert = new AssertionEngine();
                    const validation = new ValidationEngine();
                    const started = Date.now();

                    const rawResponse = await unauthenticatedApi.get(
                        PERCENTAGE_LOADING_DETAILS_PATH,
                        {
