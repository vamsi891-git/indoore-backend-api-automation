...
const CATEGORY_DISTRIBUTION_PATH =
    ...
    test(
      ...
      async ({ authenticatedApi }) => {
        ...
        if (testCase.isContractFixture) {
          const fixtureBody = resolveConsumerCategoryDistributionContractBody(
            testCase.scenario,
          );
          if (!fixtureBody) {
            test.skip(
              true,
              "Missing consumer category-distribution contract body",
            );
            return;
          }
        ...
        const query = resolveConsumerCategoryDistributionQuery(
          testCase.scenario,
        );
    ...
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        if (
          ...
          authTest.skip(
            true,
            `Rate limited (429) on ${CATEGORY_DISTRIBUTION_PATH} — retry later`,
          );
          return;
        }
