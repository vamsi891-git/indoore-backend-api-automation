...
const LOAD_UNBALANCE_DETAILS_PATH =
            ...
            async ({ authenticatedApi }) => {
                ...
                if (testCase.isContractFixture) {
                    ...
                    if (!fixtureBody) {
                        test.skip(
                            true,
                            "Missing DTR load-unbalance-details contract body",
                        );
                        return;
                    }

                    const mapped =
                        DtrLoadUnbalanceDetailsMapper.map(fixtureBody);
                ...
                const query = resolveDtrLoadUnbalanceDetailsQuery(
                    testCase.scenario,
                );
                ...
                const mapped =
                    DtrLoadUnbalanceDetailsMapper.map(responseBody);
        ...
        authTest(
            ...
            async ({ unauthenticatedApi }) => {
                ...
                if (
                    ...
                    authTest.skip(
                        true,
                        `Rate limited (429) on ${LOAD_UNBALANCE_DETAILS_PATH} — retry later`,
                    );
                    return;
                }
