    ...
    test(
      ...
      async ({ authenticatedApi }) => {
        ...
        if (testCase.isContractFixture) {
          const fixtureBody = resolveConsumerPhaseDistributionContractBody(
            testCase.scenario,
          );
          if (!fixtureBody) {
            test.skip(
              true,
              "Missing consumer phase-distribution contract body",
            );
            return;
          }
    ...
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        if (
          ...
          authTest.skip(
            true,
            `Rate limited (429) on ${PHASE_DISTRIBUTION_PATH} — retry later`,
          );
          return;
        }
