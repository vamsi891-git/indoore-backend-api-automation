...
test.describe("Dashboard — revenue subsidy (power factor)", () => {
  ...
  for (const testCase of revenueSubsidyPfTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const saveRequest = resolveRevenueSubsidyPfSaveRequest(testCase.scenario);

      if (testCase.isContractFixture) {
        const fixtureBody = resolveRevenueSubsidyPfContractBody(testCase.scenario);
        if (!fixtureBody) {
        ...
        validation.execute("Contract Scenario", () =>
          validator.validateScenario(mapped, testCase.scenario, saveRequest),
        );
      ...
      const { rawResponse, responseBody, responseTime } = await api.getRevenueSubsidyPf(
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, revenueSubsidyPfMaxResponseTimeMs),
      );
      ...
      validation.execute("Response Envelope", () =>
        validator.validateResponseEnvelope(responseBody, revenueSubsidyPfSuccessMessage),
      );
  ...
  for (const authCase of dtrUnbalanceAuthNegativeCases) {
    authTest(
      ...
      async ({ unauthenticatedApi }) => {
        ...
        const rawResponse = await unauthenticatedApi.get(REVENUE_SUBSIDY_PF_PATH, {
        ...
        if (
          ...
          authTest.skip(true, `Rate limited (429) on ${REVENUE_SUBSIDY_PF_PATH} — retry later`);
          return;
        }
        ...
        validation.execute("Auth Error Envelope", () => {
          if (authCase.expectedErrorCode === dtrUnbalanceAccessTokenInvalidCode) {
            ...
            validator.validateUnauthorizedError(responseBody as RevenueSubsidyPfErrorResponse);
          }
        });

        validation.printSummary(`Revenue subsidy PF ${authCase.testName}`, responseTime);
      },
    );
  }
});
