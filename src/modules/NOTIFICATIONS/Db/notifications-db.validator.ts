...
export function collectNotificationsDataQualityFindings(
  kind: string,
...
export async function logNotificationsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectNotificationsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `NOTIFICATIONS ${kind} DQ`);
  return report;
}
