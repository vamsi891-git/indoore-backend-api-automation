...
test.describe("Notifications CRUD Flow", () => {
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const validateJsonCall = (label: string, result: ApiCallResult, expectedStatus = 200) => {
        validation.execute(`${label} Status Code`, () =>
          assert.validateStatusCode(result.rawResponse, expectedStatus),
        );
        validation.execute(`${label} Content Type`, () =>
          assert.validateContentType(result.rawResponse),
        );
        validation.execute(`${label} Response Time`, () =>
          assert.validateResponseTime(result.responseTime, NotificationsData.maxResponseTime),
        );
        validation.execute(`${label} Sensitive Data`, () =>
          assert.validateSensitiveData(result.responseBody),
        );
      };
      const trackPerformance = async (label: string, result: ApiCallResult) => {
      ...
      // =====================================
      let notificationsResponse = await notificationApi.getNotifications(
        NotificationsData.page,
        NotificationsData.limit,
      );
      let notificationsList = NotificationsMapper.mapNotificationsList(
      ...
      if (notificationsList.total < NotificationsData.seedCount) {
        while (notificationsList.total < NotificationsData.seedCount) {
          const seedResponse = await notificationApi.createNotification(
            NotificationsMapper.buildSeedPayload(userId, notificationsList.total),
          );
          seedResults.push(seedResponse);
          if (seedResponse.rawResponse.ok()) {
            validation.execute("Validate Seed Notification Response", () =>
              validator.validateCreateResponse(seedResponse.responseBody),
            );
          } else {
            break;
          }
          notificationsResponse = await notificationApi.getNotifications(
            NotificationsData.page,
            NotificationsData.limit,
          );
          notificationsList = NotificationsMapper.mapNotificationsList(
            notificationsResponse.responseBody,
          );
        }
      }
      if (notificationsList.total < NotificationsData.seedCount) {
        const profileSeedResponse =
          await notificationApi.triggerNotificationViaProfileUpdate(userId);
        seedResults.push(profileSeedResponse);
        if (profileSeedResponse.rawResponse.ok()) {
          await new Promise((resolve) => setTimeout(resolve, NotificationsData.seedSettleMs));
          notificationsResponse = await notificationApi.getNotifications(
            NotificationsData.page,
            NotificationsData.limit,
          );
      ...
      validation.execute("Validate Notifications Response", () =>
        validator.validateResponse(notificationsResponse.responseBody),
      );
      const statsResponse = await notificationApi.getNotificationStats();
      validateJsonCall("Stats", statsResponse);
      const stats = NotificationsMapper.mapStats(statsResponse.responseBody);
      validation.execute("Validate Stats", () => validator.validateStats(stats));
      ...
      if (notificationsList.total === 0) {
        ...
        validation.execute("Validate No Data Scenario", () =>
          validator.validateNoDataScenario(notifications, stats, notificationsList.total),
        );
        await trackPerformance("Get Notifications", notificationsResponse);
        validation.printSummary(
          "Notifications CRUD Flow (No Data — add notifications in app for 67 checks)",
          notificationsResponse.responseTime + statsResponse.responseTime,
        );
        return;
      }
      const notifications = notificationsList.notifications;
      ...
      validation.execute("Validate Null Values", () => validator.validateNullValues(notifications));
      validation.execute("Validate NaN Values", () => validator.validateNaNValues(notifications));
      const targetNotification = notifications.find((item) => !item.isRead) ?? notifications[0];
      ...
      // =====================================
      const markReadResponse = await notificationApi.markNotificationRead(
      ...
      validation.execute("Validate Mark Read Response", () =>
        validator.validateMarkReadResponse(markReadResponse.responseBody),
      );
      await trackPerformance("Mark Notification Read", markReadResponse);
      // =====================================
      // STEP 4 — VERIFY MARK READ (LIST + STATS)
      // =====================================
      const notificationsAfterMarkReadResponse = await notificationApi.getNotifications(
        NotificationsData.page,
        NotificationsData.limit,
      );
      validateJsonCall("Notifications After Mark Read", notificationsAfterMarkReadResponse);
      const notificationsAfterMarkRead = NotificationsMapper.mapNotifications(
      ...
      const statsAfterReadResponse = await notificationApi.getNotificationStats();
      validateJsonCall("Stats After Mark Read", statsAfterReadResponse);
      const statsAfterRead = NotificationsMapper.mapStats(statsAfterReadResponse.responseBody);
      validation.execute("Validate Stats After Read", () =>
        validator.validateStatsAfterMarkRead(stats, statsAfterRead, wasUnread),
      );
      await trackPerformance("Get Notification Stats After Mark Read", statsAfterReadResponse);
      // =====================================
      // STEP 5 — MARK ALL READ
      // =====================================
      const markAllReadResponse = await notificationApi.markAllNotificationsRead();
      validateJsonCall("Mark All Read", markAllReadResponse);
      validation.execute("Validate Mark All Read Response", () =>
        validator.validateMarkAllReadResponse(markAllReadResponse.responseBody),
      );
      await trackPerformance("Mark All Notifications Read", markAllReadResponse);
      // =====================================
      // STEP 6 — VERIFY ALL READ (LIST + STATS)
      // =====================================
      const notificationsAfterAllReadResponse = await notificationApi.getNotifications(
        NotificationsData.page,
        NotificationsData.limit,
      );
      validateJsonCall("Notifications After Mark All Read", notificationsAfterAllReadResponse);
      const notificationsAfterAllRead = NotificationsMapper.mapNotifications(
        notificationsAfterAllReadResponse.responseBody,
      );
      validation.execute("Validate All Notifications Marked Read", () =>
        validator.validateAllNotificationsMarkedReadList(notificationsAfterAllRead),
      );
      await trackPerformance(
        "Get Notifications After Mark All Read",
        notificationsAfterAllReadResponse,
      );
      const statsAfterAllReadResponse = await notificationApi.getNotificationStats();
      validateJsonCall("Stats After Mark All Read", statsAfterAllReadResponse);
      ...
      if (stats.total >= NotificationsData.minNotificationsToDelete) {
        const deleteResponse = await notificationApi.deleteNotification(
          NotificationsData.notificationId,
        );
        ...
        const verifyDeleteResponse = await notificationApi.getNotifications(
          NotificationsData.page,
          NotificationsData.limit,
        );
        validateJsonCall("Notifications After Delete", verifyDeleteResponse);
        const notificationsAfterDelete = NotificationsMapper.mapNotifications(
        ...
        await trackPerformance("Get Notifications After Delete", verifyDeleteResponse);
        const statsAfterDeleteResponse = await notificationApi.getNotificationStats();
        validateJsonCall("Stats After Delete", statsAfterDeleteResponse);
        ...
        validation.execute("Validate Stats After Delete", () =>
          validator.validateStatsAfterDelete(statsAfterAllRead, statsAfterDelete),
        );
        await trackPerformance("Get Notification Stats After Delete", statsAfterDeleteResponse);
        apiResults.push(deleteResponse, verifyDeleteResponse, statsAfterDeleteResponse);
      }
      // ====================================
      // FINAL SUMMARY
      // =====================================
      const totalResponseTime = apiResults.reduce((sum, result) => sum + result.responseTime, 0);
      validation.printSummary("Notifications CRUD Flow", totalResponseTime);
    },
  );
});
