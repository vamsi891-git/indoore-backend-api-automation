...
export function collectEnergyAuditsDataQualityFindings(
  kind: string,
...
export async function logEnergyAuditsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectEnergyAuditsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `ENERGY-AUDITS ${kind} DQ`);
  return report;
}
