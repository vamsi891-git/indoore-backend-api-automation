...
export function registerLossAnalysisStatsTest(
  ...
  test(
    ...
    async ({ authenticatedApi }, testInfo: TestInfo) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getLossAnalysisStats(query);
      const defectContext = {
      ...
      try {
        ...
        validation.execute("Date Fields", () => validator.validateDateFields(data));
        validation.execute("Energy Metric Types", () => validator.validateEnergyMetricTypes(data));
        validation.execute("Non-Negative Totals", () => validator.validateNonNegativeTotals(data));
        validation.execute("Total Loss Math", () => validator.validateTotalLossMath(data));
        validation.execute("Loss Not Exceeding Input", () =>
          validator.validateLossNotExceedingInput(data),
        );
        validation.execute("Peak Loss Hour", () => validator.validatePeakLossHour(data));
        validation.execute("Lowest Loss Hour", () => validator.validateLowestLossHour(data));
        validation.execute("Peak Vs Lowest Loss", () => validator.validatePeakVsLowest(data));
