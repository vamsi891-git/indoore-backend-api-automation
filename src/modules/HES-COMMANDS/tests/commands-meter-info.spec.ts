...
test.describe("HES Commands — Meter Info", () => {
  ...
  test(
    "Validate GET /commands/meter-info/:serial — HES device metadata",
    ...
    async ({ authenticatedApi }, testInfo) => {
      ...
      validation.execute("Firmware Version", () => validator.validateFirmwareVersion(row));
  ...
  test(
    "Validate GET /commands/meter-info/:serial — unknown meter returns error",
