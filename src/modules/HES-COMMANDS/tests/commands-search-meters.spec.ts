...
test.describe("HES Commands — Search Meters", () => {
  ...
  test(
    ...
    async ({ authenticatedApi }, testInfo) => {
      ...
      validation.execute("Unique Meter IDs", () => validator.validateUniqueMeterIds(mapped.meters));
      validation.execute("All Meter Rows", () => validator.validateAllMeters(mapped.meters));
      ...
      validation.execute("Known Meter Present", () =>
        validator.validateKnownMeterPresent(mapped.meters, commandsSearchMetersData.searchMeterId),
      );
