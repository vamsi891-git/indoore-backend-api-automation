...
import { countHesCommandLogs, getHesCommandLogSpot } from "../Db/hes-commands.db";
import {
...
export async function runHesCommandsDbCoverage(
  ...
  if (spot) {
    const primaryMeter = parseSelectedMeterTokens(spot.selectedMeter)[0] ?? spot.selectedMeter;
    const dbRow = await getHesCommandLogSpot(db, spot.requestId, primaryMeter);
    validation.execute(`History spot requestId=${spot.requestId} vs hes_command_logs`, () => {
