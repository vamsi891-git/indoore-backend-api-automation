...
export function collectDtrsDataQualityFindings(
  kind: string,
...
export async function logDtrsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectDtrsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `DTRS ${kind} DQ`);
  return report;
}
