  ...
  for (const testCase of dtrStatisticsTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrStatisticsContractBody(testCase.scenario);
        if (!fixtureBody) {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getDtrStatistics(
        dtrCode,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrStatisticsMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as DtrStatisticsErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        validation.execute("Blank DTR Code Validation Error", () =>
          validator.validateBlankCodeError(responseBody as DtrStatisticsErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
      ...
      validation.execute("DTR Statistics Scenario", () =>
        validator.validateScenario(mapped, testCase.scenario, responseBody),
      );
