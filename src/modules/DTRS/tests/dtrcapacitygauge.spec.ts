  ...
  for (const testCase of dtrCapacityGaugeTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrCapacityGaugeContractBody(testCase.scenario);
        if (!fixtureBody) {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getCapacityGauge(
        dtrCode,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrCapacityGaugeMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as DtrCapacityGaugeErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        validation.execute("Blank DTR Code Validation Error", () =>
          validator.validateBlankCodeError(responseBody as DtrCapacityGaugeErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
