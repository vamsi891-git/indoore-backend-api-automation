...
import { DtrEventsMapper, type DtrEventsErrorResponse } from "../Mapper/dtrevents.mapper";
  ...
  for (const testCase of dtrEventsTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveDtrEventsContractBody(testCase.scenario);
        if (!fixtureBody) {
        ...
        validation.execute("Contract Scenario", () =>
          validator.validateScenario(mapped, testCase.scenario, query.page ?? 1, query.limit ?? 20),
        );
      ...
      const { rawResponse, responseBody, responseTime } = await api.getEvents(dtrCode, query);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, dtrEventsMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as DtrEventsErrorResponse),
        );
      ...
      if (testCase.scenario === "empty_dtr_code") {
        validation.execute("Blank DTR Code Validation Error", () =>
          validator.validateBlankCodeError(responseBody as DtrEventsErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
      ...
      if (testCase.scenario === "invalid_page") {
        validation.execute("Invalid Page Validation Error", () =>
          validator.validateInvalidPageError(responseBody as DtrEventsErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
