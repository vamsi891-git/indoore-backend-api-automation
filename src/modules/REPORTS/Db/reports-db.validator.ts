...
export function collectReportsDataQualityFindings(
  kind: string,
...
export async function logReportsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectReportsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `REPORTS ${kind} DQ`);
  return report;
}
