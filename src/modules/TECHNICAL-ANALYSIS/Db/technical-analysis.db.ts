...
export function isTechnicalAnalysisDbSqlReady(): boolean {
  return process.env.TECHNICAL_ANALYSIS_DB_SQL_READY?.trim().toLowerCase() === "true";
}
...
export async function countTechnicalConsumerMeters(pool: pg.Pool): Promise<number> {
  return (await queryScalar<number>(pool, TECHNICAL_CONSUMER_METER_UNIVERSE_SQL)) ?? 0;
}
