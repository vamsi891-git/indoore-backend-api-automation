...
test.describe("Technical summary", () => {
  ...
  for (const testCase of technicalSummaryTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }, testInfo) => {
      const expectedStatus = testCase.expectedStatus ?? 200;
      const query = resolveTechnicalSummaryQuery(testCase.scenario);
      const api = new TechnicalSummaryApi(authenticatedApi);
      const { rawResponse, responseBody, responseTime } = await api.getTechnicalSummary(query);
      ...
      try {
        ...
        validation.execute("Response Time", () =>
          assert.validateResponseTime(responseTime, technicalSummaryMaxResponseTimeMs),
        );
        ...
        if (rawResponse.status() !== 200 || !responseBody.success) {
          validation.execute("Error Envelope", () => validator.validateApiError(responseBody));
          return;
        }
        ...
        validation.execute("Month Validation", () => validator.validateMonth(mapped.month));
        validation.execute("Year Validation", () => validator.validateYear(mapped.year));
        if (query.month !== undefined && query.year !== undefined) {
          validation.execute("Query Echo", () =>
            validator.validateQueryEcho(mapped.month, mapped.year, query.month!, query.year!),
          );
        }
        ...
        if (testCase.scenario === "dev_live_primary") {
          validation.execute("Report card count", () =>
            validator.validateReportCount(mapped.reports, technicalSummaryExpectedReportCount),
          );
          ...
          mapped.reports.forEach((report) => {
            ...
            validation.execute(`${report.analysisType} household + non-household split`, () =>
              validator.validateDomesticNonDomesticSplit(report),
            );
            validation.execute(`${report.analysisType} report name`, () =>
            ...
            if (liveConfig) {
              validation.execute(`${report.analysisType} empty vs live card`, () =>
  ...
  test(
    ...
    async ({ authenticatedApi }, testInfo) => {
      ...
      if (rawResponse.status() === 500 && isTechnicalSummaryInternalError(responseBody)) {
      ...
      const powerFailure = mapped.reports.find((report) => report.analysisType === "power_failure");
      ...
      validation.execute("Power Failure non-household list matches the card", () => {
      ...
      for (const liveConfig of technicalAnalysisLiveConfigs) {
        ...
        if (liveConfig.validationType === "phase") {
          validation.execute(`${liveConfig.analysisType} phase list opened`, () => {
            expect(reportBody.success).toBeTruthy();
          });
          continue;
        }
        if (liveConfig.analysisType === "current_unbalance" && reportTotal !== card!.totalCount) {
        ...
        validation.execute(`${liveConfig.analysisType} list total matches summary card`, () => {
          expect(reportTotal).toBe(card!.totalCount);
        });
      }

      validation.finalize("Technical summary card totals match report lists", responseTime, {
        testInfo,
        defectContext: {
          module: "TECHNICAL-ANALYSIS",
          endpoint: "/indore/analysis/technical/summary",
