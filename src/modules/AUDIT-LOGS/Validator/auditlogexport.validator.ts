...
export class AuditLogExportValidator {
  validateNotJsonError(csvContent: string) {
    expect(csvContent.trim().startsWith("{"), "Export returned JSON instead of CSV").toBe(
      false,
    );
  }
  ...
  validateTargetConsistency(rows: AuditLogExportRow[]) {
    rows.forEach((row) => {
      if (row.targetId) {
        const hasTargetData =
          row.targetEmail || row.targetFullName || row.targetRoleName;
        if (!hasTargetData) {
