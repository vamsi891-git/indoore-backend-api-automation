...
export function collectAuditLogsDataQualityFindings(
  kind: string,
...
export async function logAuditLogsDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectAuditLogsDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `AUDIT-LOGS ${kind} DQ`);
  return report;
}
