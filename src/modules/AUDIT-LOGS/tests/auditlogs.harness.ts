...
import { AuditLogsMapper, type AuditLogsQuery } from "../Mapper/auditlogs.mapper";
...
export async function runAuditLogsValidation(options: {
  ...
  const { api, query, testLabel, sortDirection, requireLogs = true, expectedAction } = options;

  const { rawResponse, responseBody, responseTime } = await api.getAuditLogs(query);
  ...
  if (requireLogs) {
    validation.execute("Logs Exist", () => validator.validateAuditLogsExist(mapped));
  }
  validation.execute("Pagination", () => validator.validatePagination(mapped));
  validation.execute("Query Params", () => validator.validateQueryParams(mapped, query));
  validation.execute("Audit Log Fields", () => validator.validateAuditLogFields(mapped));
  ...
  validation.execute("IP Addresses", () => validator.validateIpAddresses(mapped));
  ...
  validation.execute("Action Filter Options", () => validator.validateActionFilterOptions(mapped));
  validation.execute("Log Actions In Filter Options", () =>
    validator.validateLogActionsInFilterOptions(mapped),
  );
  validation.execute("Display Labels", () => validator.validateDisplayLabels(mapped));
  validation.execute("Actor Label Matches Name", () =>
    validator.validateActorLabelMatchesName(mapped),
  );
  validation.execute("No Data Scenario", () => validator.validateNoDataScenario(mapped));
  if (sortDirection === "desc") {
    validation.execute("Descending Sort", () => validator.validateDescendingSort(mapped));
  } else {
    validation.execute("Ascending Sort", () => validator.validateAscendingSort(mapped));
  }
