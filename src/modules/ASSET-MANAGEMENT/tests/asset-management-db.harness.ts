...
import { AssetDtrLookupId, DtrDetailPaginationQueries } from "../Data/asset-management.common.data";
...
import { findDtrById, findDtrWithHighestConsumerCount } from "../utils/asset-management.helper";
...
export async function runAssetManagementDbCoverage(
  ...
  const orgHierarchy = OrganisationHierarchyMapper.mapData(orgResponse.responseBody.data).hierarchy;
  ...
  const spotConsumers = detail.consumers.slice(0, resolveAssetDbSampleSize());
  for (const spotConsumer of spotConsumers) {
    const dbRow = await getDtrConsumerSpotCheck(db, dtrId, spotConsumer.consumerTblRefId);
    validation.execute(`Consumer spot check id=${spotConsumer.consumerTblRefId}`, () => {
      compareConsumerSpotCheckToDb({
