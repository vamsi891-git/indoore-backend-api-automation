...
export type EventOtherTestCase = {
  testName: string;
...
function happyCases(): EventOtherTestCase[] {
  const cases: EventOtherTestCase[] = [];
  for (const reportType of reportTypes) {
    for (const period of periods) {
      for (const asset of assets) {
        ...
        cases.push({
          testName: `Other events — ${misGroupingWords(reportType)}, ${misPeriodWords(period)}, ${asset.words}`,
...
export const eventOtherTestCases: EventOtherTestCase[] = [
  ...happyCases(),
  {
    testName: "Other events — both is treated as all meters",
  ...
  {
    testName: "Other events — consumers is treated as consumer meters",
    params: {
      reportType: "phase-wise",
      period: "daily",
      assetType: "consumers",
  ...
  {
    testName: "Other events — dtrs is treated as DTR meters",
  ...
  {
    testName: "Other events — extra unused options are ignored",
    params: {
      reportType: "phase-wise",
      period: "daily",
      assetType: "all",
  ...
  {
    testName: "Other events — an invalid time range is rejected",
    params: { reportType: "phase-wise", period: "invalid_period" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Other events — an invalid grouping is rejected",
    params: { reportType: "not-a-type", period: "daily" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Other events — a blank time range is rejected",
    params: { reportType: "phase-wise", period: " " },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Other events — a missing grouping is rejected",
    params: { period: "daily", assetType: "all" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Other events — organisation 0 is rejected",
    params: {
      reportType: "phase-wise",
      period: "daily",
  ...
  {
    testName: "Other events — network 0 is rejected",
