...
export const eventTransactionQuery = {
...
export type EventTransactionTestCase = {
  testName: string;
...
function happyCases(): EventTransactionTestCase[] {
  const cases: EventTransactionTestCase[] = [];
  for (const reportType of reportTypes) {
    for (const period of periods) {
      for (const asset of assets) {
        ...
        cases.push({
          testName: `Meter transaction events — ${misGroupingWords(reportType)}, ${misPeriodWords(period)}, ${asset.words}`,
...
export const eventTransactionTestCases: EventTransactionTestCase[] = [
  ...happyCases(),
  {
    testName: "Meter transaction events — both is treated as all meters",
  ...
  {
    testName: "Meter transaction events — consumers is treated as consumer meters",
    params: {
      reportType: "phase-wise",
      period: "daily",
      assetType: "consumers",
  ...
  {
    testName: "Meter transaction events — dtrs is treated as DTR meters",
  ...
  {
    testName: "Meter transaction events — extra unused options are ignored",
    params: {
      reportType: "phase-wise",
      period: "daily",
      assetType: "all",
  ...
  {
    testName: "Meter transaction events — an invalid time range is rejected",
    params: { reportType: "phase-wise", period: "invalid_period" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Meter transaction events — an invalid grouping is rejected",
    params: { reportType: "not-a-type", period: "daily" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Meter transaction events — a blank time range is rejected",
    params: { reportType: "phase-wise", period: " " },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Meter transaction events — a missing grouping is rejected",
    params: { period: "daily", assetType: "all" },
    expectedStatus: 400,
    tags,
  },
  {
    testName: "Meter transaction events — organisation 0 is rejected",
    params: {
      reportType: "phase-wise",
      period: "daily",
  ...
  {
    testName: "Meter transaction events — network 0 is rejected",
