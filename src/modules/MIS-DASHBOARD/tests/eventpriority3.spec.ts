...
test.describe.skip("Urgency level 3 events", () => {
  eventPriorityQueries.forEach((query) => {
    test(
      misUrgencyTitle(3, query.period),
      ...
      async ({ authenticatedApi }) => {
        ...
        if (result.timeout || !result.rawResponse) {
          throw new Error(`Priority 3 ${query.period} request timed out`);
        }
        ...
        validation.printSummary(`Priority 3 ${query.period}`, result.responseTime);
      },
    );
  });
});
