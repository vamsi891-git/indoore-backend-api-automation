...
test.describe.skip("Urgency level 6 events", () => {
  eventPriorityQueries.forEach((query) => {
    test(
      misUrgencyTitle(6, query.period),
      ...
      async ({ authenticatedApi }) => {
        ...
        if (result.timeout || !result.rawResponse) {
          throw new Error(`Priority 6 ${query.period} request timed out`);
        }
        ...
        validation.printSummary(`Priority 6 ${query.period}`, result.responseTime);
      },
    );
  });
});
