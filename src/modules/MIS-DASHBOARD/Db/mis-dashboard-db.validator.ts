...
export function collectMisDashboardDataQualityFindings(
  kind: string,
...
export async function logMisDashboardDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectMisDashboardDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `MIS-DASHBOARD ${kind} DQ`);
  return report;
}
