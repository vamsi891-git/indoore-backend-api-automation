    ...
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as BillingHistoryErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        if (testCase.scenario === "invalid_billing_limit") {
          validation.execute("Invalid Billing Limit Error", () =>
            validator.validateInvalidBillingLimitError(responseBody as BillingHistoryErrorResponse),
          );
        } else {
          validation.execute("Blank Ref Validation Error", () =>
            validator.validateBlankRefError(responseBody as BillingHistoryErrorResponse),
          );
        }
        validation.printSummary(testCase.testName, responseTime);
        return;
