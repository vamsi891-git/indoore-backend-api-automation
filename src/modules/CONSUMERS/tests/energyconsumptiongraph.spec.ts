...
test.describe("Consumption chart", () => {
  ...
  for (const testCase of energyConsumptionGraphTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const expectedPeriod = resolveEnergyConsumptionGraphExpectedPeriod(testCase.scenario);

      if (testCase.isContractFixture) {
        const fixtureBody = resolveEnergyConsumptionGraphContractBody(testCase.scenario);
        if (!fixtureBody) {
          test.skip(true, "Missing energy-consumption-graph contract fixture body");
          return;
        }
      ...
      if (!consumerRef) {
        test.skip(true, "Could not resolve energy-consumption-graph route ref");
        return;
      }
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, energyConsumptionGraphMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as EnergyConsumptionGraphErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        if (testCase.scenario === "invalid_period") {
          ...
          validation.execute("Blank Ref Validation Error", () =>
            validator.validateBlankRefError(responseBody as EnergyConsumptionGraphErrorResponse),
          );
        }
        validation.printSummary(testCase.testName, responseTime);
        return;
