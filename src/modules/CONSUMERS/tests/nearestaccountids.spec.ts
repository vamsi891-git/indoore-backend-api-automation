  ...
  for (const testCase of nearestAccountIdsTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.scenario === "missing_account_id") {
        const { rawResponse, responseBody, responseTime } = await api.getNearestAccountIdsRaw();
      ...
      const { rawResponse, responseBody, responseTime } = await api.getNearestAccountIds(query);
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, nearestAccountIdsMaxResponseTimeMs),
      );
      ...
      validation.execute("Max Distance Type", () => validator.validateMaxDistanceType(mapped.data));
      ...
      validation.execute("Scenario Outcome", () =>
        validator.validateScenario(mapped, testCase.scenario, query.accountId, query.limit),
      );
