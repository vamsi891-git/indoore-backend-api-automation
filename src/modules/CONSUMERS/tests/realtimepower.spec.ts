  ...
  for (const testCase of realTimePowerTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveRealTimePowerContractBody(testCase.scenario);
        if (!fixtureBody) {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getRealTimePower(
        consumerRef,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, realTimePowerMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as RealTimePowerErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        validation.execute("Blank Ref Validation Error", () =>
          validator.validateBlankRefError(responseBody as RealTimePowerErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
