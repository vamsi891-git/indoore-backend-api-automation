...
import { EnergyFlowMapper, type EnergyFlowErrorResponse } from "../Mapper/energyflow.mapper";
...
test.describe("Energy flow chart", () => {
  ...
  for (const testCase of energyFlowTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getEnergyFlow(
        consumerRef,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, energyFlowMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as EnergyFlowErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        if (testCase.scenario === "invalid_period") {
          validation.execute("Invalid Period Validation Error", () =>
            validator.validateInvalidPeriodError(responseBody as EnergyFlowErrorResponse),
          );
        } else {
          validation.execute("Blank Ref Validation Error", () =>
            validator.validateBlankRefError(responseBody as EnergyFlowErrorResponse),
          );
        }
        validation.printSummary(testCase.testName, responseTime);
        return;
