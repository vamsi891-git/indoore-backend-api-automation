  ...
  for (const testCase of communicationStatusTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.isContractFixture) {
        const fixtureBody = resolveCommunicationStatusContractBody(testCase.scenario);
        if (!fixtureBody) {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getCommunicationStatus(
        consumerRef,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, communicationStatusMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));
      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as CommunicationStatusErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        if (testCase.scenario === "invalid_date" || testCase.scenario === "status_dd_mm_yyyy") {
          validation.execute("Invalid Date Validation Error", () =>
            validator.validateInvalidDateError(responseBody as CommunicationStatusErrorResponse),
          );
        } else {
          validation.execute("Blank Ref Validation Error", () =>
            validator.validateBlankRefError(responseBody as CommunicationStatusErrorResponse),
          );
        }
        validation.printSummary(testCase.testName, responseTime);
        return;
