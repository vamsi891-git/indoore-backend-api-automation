  ...
  for (const testCase of activationTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      if (testCase.scenario === "missing_status") {
        const { rawResponse, responseBody, responseTime } = await api.patchActivationRaw(
          consumerId,
          {},
        );
      ...
      if (testCase.scenario === "invalid_status" || testCase.scenario === "empty_status") {
        const { rawResponse, responseBody, responseTime } = await api.patchActivationRaw(
          consumerId,
          {
      ...
      if (
        ...
        const { rawResponse, responseBody, responseTime } = await api.updateActivation(consumerId, {
        ...
        validation.execute("Consumer Not Found", () =>
          validator.validateConsumerNotFound(responseBody as unknown as ActivationErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
      ...
      try {
        ...
        validation.execute("Response Time", () =>
          assert.validateResponseTime(responseTime, activationMaxResponseTimeMs),
        );
        ...
        validation.execute("Consumer Id", () => validator.validateConsumerId(consumer, consumerId));
        validation.execute("Table Ref Id", () => validator.validateTableRefId(consumer));
        ...
        validation.execute("Status Transition", () =>
          validator.validateStatusTransition(consumer, mapped.previousStatus, requestStatus),
        );
        validation.execute("Scenario Outcome", () =>
          validator.validateScenario(mapped, testCase.scenario, consumerId, requestStatus),
        );
        validation.printSummary(testCase.testName, responseTime);
      } finally {
