...
import { EventLogListMapper, type EventLogListErrorResponse } from "../Mapper/eventloglist.mapper";
...
test.describe("Event list", () => {
  ...
  for (const testCase of eventLogListTestCases) {
    test(testCase.testName, { tag: testCase.tags }, async ({ authenticatedApi }) => {
      ...
      const { rawResponse, responseBody, responseTime } = await api.getEventLogList(
        consumerRef,
        query,
      );
      ...
      validation.execute("Response Time", () =>
        assert.validateResponseTime(responseTime, eventLogListMaxResponseTimeMs),
      );
      validation.execute("Sensitive Data", () => assert.validateSensitiveData(responseBody));

      if (expectedStatus === 404) {
        validation.execute("Not Found Error", () =>
          validator.validateNotFoundError(responseBody as EventLogListErrorResponse),
        );
      ...
      if (expectedStatus === 400) {
        validation.execute("Blank Ref Validation Error", () =>
          validator.validateBlankRefError(responseBody as EventLogListErrorResponse),
        );
        validation.printSummary(testCase.testName, responseTime);
        return;
      }
