...
export function resolveNearestAccountIdsQuery(
  ...
  switch (scenario) {
    case "nearest_found":
      return {
        accountId:
          process.env.NEAREST_ACCOUNT_IDS_ACCOUNT_ID?.trim() || nearestAccountIdsDefaultAccountId,
        limit: 10,
      };
    case "default_limit":
      return {
...
export const nearestAccountIdsTestCases: NearestAccountIdsTestCase[] = [
  {
    testName: "Nearby accounts — similar account numbers are listed",
    scenario: "nearest_found",
    tags: ["@smoke", "@consumer", "@nearest-account-ids"],
    nonEmptyExpected: true,
  },
  {
    testName: "Nearby accounts — default list shows up to 10 accounts",
    scenario: "default_limit",
    tags: ["@consumer", "@nearest-account-ids", "@edge"],
    nonEmptyExpected: false,
  },
  {
    testName: "Nearby accounts — list size can be limited",
    scenario: "custom_limit",
    tags: ["@consumer", "@nearest-account-ids", "@edge"],
    nonEmptyExpected: false,
  },
  {
    testName: "Nearby accounts — very close match range can return none",
    scenario: "max_distance_empty",
    tags: ["@consumer", "@nearest-account-ids", "@negative"],
    nonEmptyExpected: false,
  },
  {
    testName: "Nearby accounts — letters-only account has no neighbours",
    scenario: "no_numeric_suffix",
    tags: ["@consumer", "@nearest-account-ids", "@negative"],
    nonEmptyExpected: false,
  },
  {
    testName: "Nearby accounts — number at the end of the account is used",
    scenario: "prefixed_account_id",
    tags: ["@consumer", "@nearest-account-ids", "@edge"],
    nonEmptyExpected: false,
  },
  {
    testName: "Nearby accounts — account number is required",
  ...
  {
    testName: "Nearby accounts — empty account number is rejected",
  ...
  {
    testName: "Nearby accounts — list size cannot be zero",
  ...
  {
    testName: "Nearby accounts — list size cannot be more than 20",
