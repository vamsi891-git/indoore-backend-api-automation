...
export class DtrSearchValidator {
  ...
  validateItemsExist(data: DtrSearchData): void {
    if (data.total > 0) {
  ...
  validateRequiredFields(data: DtrSearchData): void {
    data.item.forEach(item => {
      expect(item.dtrCode).toBeTruthy();
      expect(item.code).toBeTruthy();
      expect(item.dtrName).toBeTruthy();
      expect(item.dtr).toBeTruthy();
    });
  }
  validateDataTypes(data: DtrSearchData): void {
    data.item.forEach(item => {
      expect(typeof item.slNo).toBe("number");
      expect(typeof item.dtrCode).toBe("string");
      expect(typeof item.code).toBe("string");
      expect(typeof item.dtrName).toBe("string");
      expect(typeof item.dtr).toBe("string");
    });
  }
  validateMeterSerialNumbers(data: DtrSearchData): void {
    data.item.forEach(item => {
      if (item.meterSerialNumber !== null) {
        expect(item.meterSerialNumber.length).toBeGreaterThan(0);
      }
    });
  }
  validateDuplicateMeterSerials(data: DtrSearchData): void {
    const serials = data.item.filter(x => x.meterSerialNumber).map(x => x.meterSerialNumber);
    expect(serials.length).toBe(new Set(serials).size);
  }
  validateCoordinates(data: DtrSearchData): void {
    data.item.forEach(item => {
  ...
  validateMF(data: DtrSearchData): void {
    data.item.forEach(item => {
      if (item.mf !== null) {
        expect(Number(item.mf)).toBeGreaterThan(0);
      }
    });
  }
  validateBusinessRules(data: DtrSearchData): void {
    data.item.forEach(item => {
      expect(item.dtrCode).toEqual(item.code);
