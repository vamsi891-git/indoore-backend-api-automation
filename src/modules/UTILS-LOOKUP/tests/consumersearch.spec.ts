...
test.describe("Search Consumer API", () => {
  test(
    ...
    async ({ authenticatedApi }) => {
      ...
      const data =
        SearchConsumerMapper.mapData(
          responseBody.data
        );

      const validator =
        new SearchConsumerValidator();
      ...
      validation.execute(
        "Duplicate Meter Lookup Validation",
        () =>
          validator.validateDuplicateMeterLookupIds(
            data
          )
      );
      ...
      validation.execute(
        "Load Validation",
        () => validator.validateLoadValues(data)
      );

      validation.execute(
        "IVRS Consistency Validation",
        () => validator.validateIvrsConsistency(data)
      );

      validation.execute(
        "Installation Date Validation",
        () => validator.validateInstallationDate(data)
      );
