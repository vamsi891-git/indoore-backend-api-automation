...
function runOrganisationHierarchyValidations(
  ...
  const data = OrganisationMapper.mapData(getLookupResponseData<OrganisationData>(responseBody));
...
registerCatalogLookupTests({
  ...
  fetch: (authenticatedApi) => new OrganisationApi(authenticatedApi).getOrganisationHierarchy(),
