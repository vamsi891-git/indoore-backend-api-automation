...
test.describe("Utils lookup — contract snapshots", () => {
  ...
  for (const contractCase of utilsLookupContractCases) {
    test(contractCase.testName, { tag: contractCase.tags }, async ({ authenticatedApi }) => {
      ...
      const hasColumnsGrid = contractCase.kind === "grid" && columns.length > 0;

      const snapshot = buildLookupItemsContractSnapshot({
        pathPattern: contractCase.pathPattern,
        dataKeys: Object.keys(data),
        itemKeys: itemKeys.length > 0 ? itemKeys : columns.map((column) => column.key),
        hasColumnsGrid,
