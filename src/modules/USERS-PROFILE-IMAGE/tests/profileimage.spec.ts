...
import {PROFILE_IMAGE_TEST_BYTES,putImageToPresignedUrl,} from "../utils/profileimage-upload.util";
test.describe("User Profile Image Flow", () => {
    test.describe.configure({ mode: "serial" });
    test("Validate profile image upload, save, and delete lifecycle",
        ...
        async ({ authenticatedApi }) => {
            ...
            validation.execute("Upload URL Response Time", () =>
                assert.validateResponseTime(uploadResponse.responseTime,ProfileImageData.maxResponseTime,),
            );
            ...
            validation.execute("Save Profile Image Status Code", () =>
                assert.validateStatusCode(saveResponse.rawResponse,200,saveResponse.responseBody,),
            );
            ...
            validation.execute("Save Success Message", () =>
                validator.validateSuccessMessage(saved.message,ProfileImageData.updatedMessage,),
            );
            ...
            validation.execute("Validate Profile Image Saved", () =>
                validator.validateProfileImageSaved(saved.user,uploadData.fileUrl,uploadData.key,),
            );
            ...
            validation.execute("Delete Success Message", () =>
                validator.validateSuccessMessage(removed.message,ProfileImageData.removedMessage,),
            );
            validation.execute("Validate Profile Image Removed", () =>
                validator.validateProfileImageRemoved(removed.user),
            );
            await PerformanceTracker.track(uploadResponse.rawResponse,"Profile Image Upload URL",
            uploadResponse.rawResponse.url(),uploadResponse.responseTime,);
            await PerformanceTracker.track(saveResponse.rawResponse,"Profile Image Save",saveResponse.rawResponse.url(),saveResponse.responseTime,);
            await PerformanceTracker.track(deleteResponse.rawResponse,"Profile Image Delete",deleteResponse.rawResponse.url(),deleteResponse.responseTime,);
            assert.assertValidationResults(validation.getResults());
            validation.printSummary("User Profile Image Module",
