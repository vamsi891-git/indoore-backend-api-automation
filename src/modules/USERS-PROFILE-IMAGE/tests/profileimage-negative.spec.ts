...
test.describe("User Profile Image — Negative", () => {
    ...
    test("POST /users/me/profile-image/upload-url — missing fileName returns 400",
        ...
        async ({ authenticatedApi }) => {
            ...
            validation.printSummary("Upload URL — Missing fileName",responseTime,);
        },
    );
    test("POST /users/me/profile-image/upload-url — missing fileSize returns 400",
        { tag: ["@negative", "@users-profile-image"] },
        async ({ authenticatedApi }) => {
            const assert = new AssertionEngine();
            const validation = new ValidationEngine();
            ...
            validation.printSummary("Upload URL — Missing fileSize",responseTime,);
        },
    );
    test("PATCH /users/me/profile-image — invalid profileImageUrl returns 400",
        { tag: ["@negative", "@users-profile-image"] },
        async ({ authenticatedApi }) => {
            const assert = new AssertionEngine();
            const validation = new ValidationEngine();
    ...
    test("PATCH /users/me/profile-image — empty body returns 400",
        { tag: ["@negative", "@users-profile-image"] },
        async ({ authenticatedApi }) => {
            const assert = new AssertionEngine();
            const validation = new ValidationEngine();
