...
test.describe("User Profile Image — List", () => {
    test("Validate POST /users/me/profile-image/upload-url — presigned upload contract",
        ...
        async ({ authenticatedApi }) => {
            ...
            validation.execute("Upload URL Response Time", () =>
                assert.validateResponseTime(response.responseTime,ProfileImageData.maxResponseTime,
                ),
            );
            validation.execute("Upload URL Sensitive Data", () =>
                assert.validateSensitiveData(response.responseBody),
            );
            await PerformanceTracker.track(response.rawResponse,"Profile Image Upload URL",response.rawResponse.url(),response.responseTime,
            );
