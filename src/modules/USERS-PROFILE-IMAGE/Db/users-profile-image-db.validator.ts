...
export function collectUsersProfileImageDataQualityFindings(
  kind: string,
...
export async function logUsersProfileImageDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectUsersProfileImageDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `USERS-PROFILE-IMAGE ${kind} DQ`);
  return report;
}
