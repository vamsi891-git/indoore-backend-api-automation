...
export function collectCommericialAnalysisDataQualityFindings(
  kind: string,
...
export async function logCommericialAnalysisDataQualityFindings(
  kind: string,
  data: Record<string, unknown> | null | undefined,
): Promise<DataQualityReport> {
  const report = collectCommericialAnalysisDataQualityFindings(kind, data);
  await attachDataQualityReport(report, `COMMERICIAL-ANALYSIS ${kind} DQ`);
  return report;
}
