...
function shouldUpdateSnapshots(): boolean {
  return env.UPDATE_CONTRACT_SNAPSHOTS;
}
...
function snapshotPath(snapshotName: string): string {
  ...
  if (safe.split(/[/\\]/).includes("..")) {
    throw new Error(`[contract-snapshot] snapshotName must not contain "..": ${snapshotName}`);
  }
  ...
  if (
    ...
    for (const key of [...keys].sort()) {
      ...
      if (!hasExpected) {
        ...
        diffs.push(...diffContractValues(expectedObj[key], actualObj[key], childPath));
      }
    }
    return diffs;
  }
...
export function formatContractDiff(diffs: ContractDiffEntry[]): string {
  ...
  const lines = [`Contract snapshot mismatch — ${diffs.length} difference(s):`, ""];
 ...
 */
export async function assertContractSnapshot(snapshotName: string, actual: unknown): Promise<void> {
  ...
  if (isNew || update) {
    ...
    if (isNew) {
      console.warn(`[contract-snapshot] Created new snapshot at: ${relativePath}`);
    } else {
      console.warn(
