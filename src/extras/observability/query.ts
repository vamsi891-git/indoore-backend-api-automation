 ...
 *
 * Usage:
 *   ts-node --compilerOptions '{"module":"commonjs"}' src/extras/observability/query.ts <runId> [--test "<testId>"]
 *   ts-node ... src/extras/observability/query.ts --latest
 *   ts-node ... src/extras/observability/query.ts --run <runId> --json
 */
...
function groupByTest(events: ObservabilityEvent[]): Map<string, ObservabilityEvent[]> {
  ...
  for (const event of events) {
    ...
    const key =
      event.kind === "ci_routing" && !event.testId ? "<run>" : event.testId || "<unknown>";
...
function main(): void {
  ...
  if (events.length === 0) {
    console.error(
      `No events found for runId=${runId}${args.testId ? ` testId=${args.testId}` : ""}`,
    );
    process.exitCode = 1;
    return;
  }
