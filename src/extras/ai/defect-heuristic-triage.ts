 ...
 */
import type { ValidationResult } from "../../core/models/result.model";
import type { DefectReportContext, DefectTriage, DefectTriageResult } from "./defect-triage.types";
...
export function buildHeuristicTriage(input: HeuristicTriageInput): DefectTriageResult {
  ...
  if (status !== undefined && status >= 500) {
    triage = {
      ...
      classification: code === "INTERNAL_ERROR" || status === 500 ? "backend_crash" : "flaky_infra",
      likelyOwner: "backend",
      confidence: 0.9,
      summary:
        code === "INTERNAL_ERROR"
    ...
    triage = {
      ...
      likelyOwner: "backend",
      confidence: 0.75,
      summary: "Route or entity not found. Path typo, env mismatch, or missing seed data.",
      nextSteps: [
    ...
    triage = {
      ...
      likelyOwner: failed.some((f) => /status code/i.test(f.name)) ? "qa" : "backend",
      confidence: 0.7,
      summary: "Request rejected as invalid, or test expected a different validation outcome.",
      nextSteps: [
