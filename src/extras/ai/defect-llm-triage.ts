...
export function isDefectLlmEnabled(): boolean {
  if (!env.DEFECT_LLM_ENABLED) {
    return false;
  }
  return Boolean(env.DEFECT_LLM_API_KEY || env.OPENAI_API_KEY);
}
 ...
 */
export async function enrichDefectTriage(input: HeuristicTriageInput): Promise<DefectTriageResult> {
  ...
  const apiKey = env.DEFECT_LLM_API_KEY || env.OPENAI_API_KEY || "";
  const baseUrl = (env.DEFECT_LLM_BASE_URL || "https://api.openai.com/v1").replace(/\/$/, "");
  const model = env.DEFECT_LLM_MODEL || "gpt-4o-mini";
  const timeoutMs = env.DEFECT_LLM_TIMEOUT_MS ?? 20_000;
