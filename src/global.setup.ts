...
async function globalSetup(): Promise<void> {
  ...
  // Flow: restore warm token → else CI bearer token → else captcha+OTP login once
  ...
  if (cachedSession) {
    ...
    console.error(
      "Cached access token rejected by GET /auth/me (expired or revoked). Continuing auth bootstrap.",
    );
    LoggerEngine.info("Global setup discarded stale cached auth token");
  }

  console.error(
    "Global setup: login once for this suite (passkey-as-captcha or OCR + OTP). Remaining tests reuse the session.",
  );
